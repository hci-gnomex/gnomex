...
public class RemoteSessionManager implements GUIControlChannelNotifier, Runnable {
	...
	public String getFileSeparator() {
		synchronized (lock) {
			while (!initiated) {
				try { lock.wait(timeout); } catch (Throwable t) { }
			}
		}
...
//			channel.sendCtrlMessage(c);
	...
	public void removeFiles(String files[]) throws Exception {
		...
		synchronized (lock) {
			while (fileList == null) {
				try { lock.wait(timeout); } catch (Throwable a) { }
			}
		}
			...
			public void run() {
				...
				if (auth) {
					try {
						...
						try { channel.close(t.getLocalizedMessage(), t); } catch (Throwable tt) { }
						channel = null;
						t.printStackTrace();
						initiated = true;
					}
...
//			Thread.currentThread().setName("RemoteSessionMonitor[GUI]");
			if (channel == null || channel.isClosed()) {
				...
				try { channel.close("Closed from the other end", new Exception()); } catch (Throwable tt) { }
				channel = null;
			}
		}
	}
