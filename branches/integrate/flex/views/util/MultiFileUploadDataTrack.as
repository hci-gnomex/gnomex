...
//	EXAMPLE:
//
//	MultiFileUploadDataTrack= new MultiFileUpload(
...
package views.util 
{
    ...
    public class MultiFileUploadDataTrack{
        ...
        //Constructor    
        public function MultiFileUploadDataTrack(
        								dataGrid:DataGrid,
        								browseButton:Button,
       ...
       *********************************************************/
		...
		{
			var results:XML = XML(dataCompleteEvent.data);
			if (results.name() == 'ERROR') {
				if (_files.length > 1) {
					mx.controls.Alert.show(results.@message + "\nDo you want to proceed with the remaining uploads?",
						"Continue?",
						(Alert.YES | Alert.NO), 
						null,
        ...
        private function httpStatusHandler(event:HTTPStatusEvent):void {
			mx.controls.Alert.show("File '" + event.target.name + "' did not upload.",
				"HTTP Status error " + event.status, 0);
        }
