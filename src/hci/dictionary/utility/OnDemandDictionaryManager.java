 ...
 * @since 11/4/2005
 */
public final class OnDemandDictionaryManager implements DictionaryActions, Serializable {
  private static HashMap<String, OnDemandDictionaryManager> managerMap = new HashMap<>();
  ...
  private OnDemandDictionaryManager() {
   ...
   */
  public static synchronized OnDemandDictionaryManager getDictionaryManager(String dictionaryXmlFile, Session sess, Object o) {
   ...
   *         with the cached dictionaries
   */
  public static synchronized OnDemandDictionaryManager getDictionaryManager(String dictionaryXmlFile, Session sess, Object o, boolean includeNullEntry) {

    OnDemandDictionaryManager manager = null;

    if (managerMap.containsKey(dictionaryXmlFile)) {
      manager = (OnDemandDictionaryManager) managerMap.get(dictionaryXmlFile);
    } else {
      manager = new OnDemandDictionaryManager();
    ...
    manager.includeNullEntry = includeNullEntry;

    return manager;
  }

  private Dictionary getDictionary(String className, Session sess) {
    Dictionary dictionary = (Dictionary) dictionaries.get(className);
    if (dictionary != null) {
      if (dictionary.dictionaryEntries == null || dictionary.dictionaryEntries.isEmpty()) {
        this.loadDictionary(dictionary, sess, includeNullEntry);
      }
    }
    return dictionary;
  }
   ...
   *         dictionary
   */
  private String getDictionaryXML(String className, SecurityAdvisor securityAdvisor, Session sess) {
    synchronized (this) {
      StringBuffer r = new StringBuffer("<Dictionaries>");

      try {
        if (dictionaries != null && dictionaries.get(className) != null) {
          Dictionary dict = getDictionary(className, sess);
          dict.setSecurity(securityAdvisor);
          String xml = dict.toDictionaryXML();
          r.append(xml.substring((xml.indexOf('>') + 1)));

          // Also return XML for any filter dictionaries
          if (dict.getFilters() != null) {
            Iterator<?> i = dict.getFilters().values().iterator();
            while (i.hasNext()) {
              Filter f = (Filter) i.next();
              Dictionary fDict = getDictionary(f.filterClass, sess);
              fDict.setSecurity(securityAdvisor);
              xml = fDict.toDictionaryXML();
              r.append(xml.substring((xml.indexOf('>') + 1)));
            }
          }
        }
  ...
  public static Set<DictionaryEntry> getDictionaryEntries(String className, Session sess) {
    Set<DictionaryEntry> entries = null;

    Iterator<?> i = managerMap.values().iterator();
    while (i.hasNext()) {
      OnDemandDictionaryManager dm = (OnDemandDictionaryManager) i.next();
      if (dm.dictionaries != null && dm.dictionaries.get(className) != null) {
        Dictionary dict = dm.getDictionary(className, sess);
  ...
  private static Map<String, DictionaryEntry> getDictionaryEntryMap(String className, Session sess) {
    Map<String, DictionaryEntry> entries = null;
  ...
  public static String getDisplay(String className, String key, Session sess) {
    String display = null;
    Map<String, DictionaryEntry> entries = getDictionaryEntryMap(className, sess);
    if (entries != null) {
  ...
  public static String getDisplay(String className, DictionaryEntry de, Session sess) {
    String display = null;
    Map<String, DictionaryEntry> entries = getDictionaryEntryMap(className, sess);
    if (entries != null) {
  ...
  public static boolean valueEquals(String className, String key, String display, Session sess) {
    String realDisplay = getDisplay(className, key, sess);
    if (realDisplay != null) {
  ...
  public static boolean valueEquals(String className, DictionaryEntry de, String display, Session sess) {
    String realDisplay = getDisplay(className, de, sess);
    if (realDisplay != null) {
      return realDisplay.equals(display);
    } else {
      return false;
  ...
  public String getValue(String className, String display, Session sess) {
    String value = null;
    Set<DictionaryEntry> entries = getDictionaryEntries(className, sess);
    if (entries != null) {
  ...
  public void loadCommand(DictionaryCommand command, HttpServletRequest request) {
    ...
    if (LOAD_XML.equals(command.action) || OPEN_EDITOR.equals(command.action) || LOAD_METADATA.equals(command.action) || "reload".equals(command.action)) {
  ...
  public void executeCommand(DictionaryCommand command, Session sess, SecurityAdvisor securityAdvisor, boolean includeNullEntry) throws HibernateException {
    ...
    synchronized (this) {
      ...
      if (LOAD_XML.equals(command.action)) {
        ...
        if (command.className != null && !command.className.equals("")) {
          command.xmlResult = this.getDictionaryXML(command.className, securityAdvisor, sess);
        }
      ...
      // LOAD DICTIONARY METADATA FOR FLEX
        ...
        if (command.className != null && !command.className.equals("")) {
          Dictionary dict = getDictionary(command.className, sess);
          ...
          if (securityAdvisor.canUpdate(command.dictionaryEntry)) {
            ...
            try {
              ...
              try {
                ...
                // Update the cached dictionary entry
                dict = getDictionary(command.className, sess);
      ...
      else if ("reload".equals(command.action)) {
        if (command.className != null && !command.className.equals("")) {
          Dictionary dict = getDictionary(command.className, sess);
          this.loadDictionary(dict, sess, false);
          command.xmlResult = this.getDictXML(securityAdvisor, dict);
        }
      }
    }
   ...
   */
  public String getDisplayByValue(String className, Object key, Session sess) {
    if (key != null) {
      Dictionary dict = getDictionary(className, sess);
  ...
  private class Dictionary extends HibernateDetailObject {
    ...
    private TreeMap<String, DictionaryEntry> dictionaryEntries;
    private HashMap<String, Filter> filters;
    private ArrayList<Filter> filterList;
    private HashMap<String, Integer> maxLengths;
    ...
    public TreeMap<String, DictionaryEntry> getDictionaryEntryMap() {
    ...
    public TreeSet<DictionaryEntry> getDictionaryEntries() {
      ...
      if (dictionaryEntries != null) {
        Iterator<?> i = dictionaryEntries.values().iterator();
        while (i.hasNext()) {
          DictionaryEntry de = (DictionaryEntry) i.next();

          if (sortedEntries == null) {
            sortedEntries = new TreeSet<>(de);
          }
    ...
    public void setSecurity(SecurityAdvisor securityAdvisor) {
      ...
      if (this.dictionaryEntries != null) {
        Iterator<?> i = this.dictionaryEntries.values().iterator();
        while (i.hasNext()) {
          DictionaryEntry de = (DictionaryEntry) i.next();
    ...
    public boolean getCanWriteFilters(DictionaryEntry de, SecurityAdvisor securityAdvisor) throws UnknownPermissionException {
      ...
      if (this.getFilters() != null) {
        Iterator<?> i = this.getFilters().values().iterator();
        while (i.hasNext()) {
          Filter f = (Filter) i.next();
    ...
    public String toDictionaryXML() throws XMLReflectException {
      ...
      try {
        ...
        Iterator<?> fIter = filterList.iterator();
        while (fIter.hasNext()) {
        ...
        Iterator<?> i = this.getDictionaryEntries().iterator();
    ...
    private String createEditorMetaData(Session sess) {
      ...
      if (this.className != null) {
        try {
          ...
          List<String> propertyNames = Arrays.asList(meta.getPropertyNames());
