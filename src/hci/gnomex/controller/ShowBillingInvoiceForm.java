			...
			if (secAdvisor.hasPermission(SecurityAdvisor.CAN_MANAGE_BILLING)) {
				...
				if (idLabs != null) {
					...
					for (int i = 0; i < labs.length; i++) {
						...
						cacheBillingItemMaps(sess, secAdvisor, idBillingPeriod, labs[i].getIdLab(),
								billingAccounts[i].getIdBillingAccount(), idCoreFacility, billingItemMap[i],
								relatedBillingItemMap[i], requestMap[i]);
					}
				} else {
					multipleLabs = false;
					...
					cacheBillingItemMaps(sess, secAdvisor, idBillingPeriod, idLab, idBillingAccount, idCoreFacility,
							billingItemMap[0], relatedBillingItemMap[0], requestMap[0]);
				}
				...
				// Build PDF elements
				BillingPDFFormatter formatter = new BillingPDFFormatter(sess, billingPeriod, coreFacility,
						multipleLabs, lab, labs, billingAccount, billingAccounts, PropertyDictionaryHelper.getInstance(
								sess).getCoreFacilityProperty(idCoreFacility,
								PropertyDictionary.CONTACT_ADDRESS_CORE_FACILITY), PropertyDictionaryHelper
								.getInstance(sess).getCoreFacilityProperty(idCoreFacility,
										PropertyDictionary.CONTACT_REMIT_ADDRESS_CORE_FACILITY),
...
public static File makePDFBillingInvoice(Session sess, String serverName, BillingPeriod billingPeriod,
	...
	BillingPDFFormatter formatter = new BillingPDFFormatter(sess, billingPeriod, coreFacility, multipleLabs, lab, labs,
			...
			PropertyDictionaryHelper.getInstance(sess).getCoreFacilityProperty(coreFacility.getIdCoreFacility(),
...
public static void cacheBillingItemMaps(Session sess, SecurityAdvisor secAdvisor, Integer idBillingPeriod,
		Integer idLab, Integer idBillingAccount, Integer idCoreFacility, Map billingItemMap, Map relatedBillingItemMap,
		Map requestMap) throws Exception {
	cacheRequestBillingItemMap(sess, secAdvisor, idBillingPeriod, idLab, idBillingAccount, idCoreFacility,
			billingItemMap, relatedBillingItemMap, requestMap);
	cacheDiskUsageBillingItemMap(sess, secAdvisor, idBillingPeriod, idLab, idBillingAccount, idCoreFacility,
			billingItemMap, relatedBillingItemMap, requestMap);
	cacheProductOrderBillingItemMap(sess, secAdvisor, idBillingPeriod, idLab, idBillingAccount, idCoreFacility,
			billingItemMap, relatedBillingItemMap, requestMap);
}

private static void cacheRequestBillingItemMap(Session sess, SecurityAdvisor secAdvisor, Integer idBillingPeriod,
		Integer idLab, Integer idBillingAccount, Integer idCoreFacility, Map billingItemMap, Map relatedBillingItemMap,
		Map requestMap) {
	...
	buf.append("AND    bi.codeBillingStatus in ('" + BillingStatus.COMPLETED + "', '" + BillingStatus.APPROVED + "', '"
			+ BillingStatus.APPROVED_CC + "', '" + BillingStatus.APPROVED_PO + "')");
...
private static void cacheDiskUsageBillingItemMap(Session sess, SecurityAdvisor secAdvisor, Integer idBillingPeriod,
		Integer idLab, Integer idBillingAccount, Integer idCoreFacility, Map billingItemMap, Map relatedBillingItemMap,
		Map requestMap) {
	StringBuffer buf = new StringBuffer();
...
private static void cacheProductOrderBillingItemMap(Session sess, SecurityAdvisor secAdvisor, Integer idBillingPeriod,
		Integer idLab, Integer idBillingAccount, Integer idCoreFacility, Map billingItemMap, Map relatedBillingItemMap,
		Map requestMap) {
	StringBuffer buf = new StringBuffer();
...
private static boolean determineContainsPendingItems(Set set, Integer idBillingPeriod, Integer idBillingAccount) {
	for (Iterator iter = set.iterator(); iter.hasNext();) {
		...
		if (item.getIdBillingPeriod().equals(idBillingPeriod) && item.getIdBillingAccount().equals(idBillingAccount)) {
