...
implements Serializable {
  ...
  public void loadCommand( HttpServletRequest request, HttpSession session ) {
    ...
    if( rcScreen.getCodeRequestCategory() == null
        || rcScreen.getCodeRequestCategory().equals( "" ) ) {
    ...
    if( request.getParameter( "type" ) == null
      ...
      this.addInvalidField( "Null Platform Type",
          "The Experiment Platform type cannot be null" );
    }

    if( request.getParameter( "customWarningMessage" ) != null
        && ! request.getParameter( "customWarningMessage" ).equals( "" ) ) {
      this.customWarningMessage = request.getParameter(
          "customWarningMessage" );
    }

    if( request.getParameter( "noProductsMessage" ) != null
        && ! request.getParameter( "noProductsMessage" ).equals( "" ) ) {
    ...
    if( request.getParameter( "requireNameDescription" ) != null
        && ! request.getParameter( "requireNameDescription" ).equals( "" ) ) {
      this.requireNameDescription = request.getParameter(
          "requireNameDescription" );
    }

    if( request.getParameter( "sampleTypesXMLString" ) != null
        && ! request.getParameter( "sampleTypesXMLString" ).equals( "" ) ) {
      ...
      try {
        ...
        this.addInvalidField( "sampleTypesXMLString",
            "Invalid sampleTypesXMLString" );
      }
    }

    if( request.getParameter( "prepTypesXMLString" ) != null
        && ! request.getParameter( "prepTypesXMLString" ).equals( "" ) ) {
      ...
      try {
        ...
        this.addInvalidField( "prepTypesXMLString",
            "Invalid prepTypesXMLString" );
      }
    }

    if( request.getParameter( "applicationsXMLString" ) != null
        && ! request.getParameter( "applicationsXMLString" ).equals( "" ) ) {
      ...
      try {
        ...
        this.addInvalidField( "applicationsXMLString",
            "Invalid applicationsXMLString" );
      }
    }

    if( request.getParameter( "sequencingOptionsXMLString" ) != null
        && ! request.getParameter( "sequencingOptionsXMLString" ).equals(
            "" ) ) {
      sequencingOptionsXMLString = request.getParameter(
          "sequencingOptionsXMLString" );
      ...
      try {
        ...
        this.addInvalidField( "sequencingOptionsXMLString",
            "Invalid sequencingOptionsXMLString" );
      }

      for( Iterator i = this.sequencingOptionsDoc.getRootElement().getChildren().iterator(); i.hasNext(); ) {
        Element node = ( Element ) i.next();
        try {
          Integer ti = Integer.valueOf(
              node.getAttributeValue( "idNumberSequencingCycles" ) );
          if( ti < 0 ) {
            this.addInvalidField( "sequencingOptions.idNumberSequencingLanes",
                "Invalid number sequencing lanes" );
          }
        ...
        try {
          Integer ti = Integer.valueOf(
              node.getAttributeValue( "idSeqRunType" ) );
          if( ti < 0 ) {
            this.addInvalidField( "sequencingOptions.idSeqRunType",
                "Invalid sequence run type" );
          }
        } catch( NumberFormatException e ) {
          this.addInvalidField( "sequencingOptions.idSeqRunType",
              "Invalid sequence run type" );
        ...
        if( name == null || name.length() == 0 ) {
          this.addInvalidField( "sequencingOptions.name",
              "Sequence option must have a non-blank name" );
        }
      }
    }

    if( request.getParameter( "requestCategoryApplicationXMLString" ) != null
        && ! request.getParameter(
            "requestCategoryApplicationXMLString" ).equals( "" ) ) {
      String requestCategoryApplicationXMLString = request.getParameter(
          "requestCategoryApplicationXMLString" );
      ...
      try {
        ...
        this.addInvalidField( "requestCategoryApplicationXMLString",
            "Invalid requestCategoryApplicationXMLString" );
      }
    }
  ...
  public Command execute() throws RollBackCommandException {
    ...
    try {
      ...
      if( this.getSecurityAdvisor().hasPermission(
          SecurityAdvisor.CAN_SUBMIT_REQUESTS ) ) {

        if( newCodeRequestCategory != null
            && ! newCodeRequestCategory.equals( "" ) ) {
          List requestCategories = sess.createQuery(
              "Select rc.codeRequestCategory from RequestCategory as rc" ).list();
          for( Iterator i1 = requestCategories.iterator(); i1.hasNext(); ) {
            String crc = ( String ) i1.next();
            if( crc.equals( newCodeRequestCategory ) ) {
              this.addInvalidField( "Duplicate Code Request Category",
                  "The Code Request Category you selected is already in use.  Please select another." );
        ...
        if( isNewRequestCategory ) {
          ...
          if( newCodeRequestCategory != null
            ...
            rc.setCodeRequestCategory(
                "EXP" + this.getNextAssignedRequestCategoryNumber( sess ) );
          }
          sess.save( rc );
        } else {
          rc = ( RequestCategory ) sess.load( RequestCategory.class,
              rcScreen.getCodeRequestCategory() );
        ...
        // now check and see if we need to create a sample warning property for
        // sample batch size
        updateProperty( sess, PropertyDictionary.PROPERTY_SAMPLE_BATCH_WARNING,
            customWarningMessage,
        ...
        updateProperty( sess, PropertyDictionary.PROPERTY_NO_PRODUCTS_MESSAGE,
            noProductsMessage,
        ...
        updateProperty( sess, PropertyDictionary.STATUS_TO_USE_PRODUCTS,
            productStatus,
        ...
        updateProperty( sess,
            PropertyDictionary.DESCRIPTION_NAME_MANDATORY_FOR_INTERNAL_EXPERIMENTS,
            requireNameDescription,
            "Makes the experiment name and description mandatory fields.", "N",
            idCoreFacility, codeRequestCategory );
        ...
        if( unableToDelete ) {
          this.xmlResult = "<SUCCESS codeRequestCategory=\""
              + rc.getCodeRequestCategory()
              + "\" unableToDelete=\"Certain prep types were marked as inactive instead of deleted because there are requests that are associated with the given prep type\"/>";
        } else {
          this.xmlResult = "<SUCCESS codeRequestCategory=\""
              + rc.getCodeRequestCategory() + "\"/>";
        }
        ...
        this.addInvalidField( "Insufficient permissions",
            "Insufficient permission to save experiment platform." );
        setResponsePage( this.ERROR_JSP );
      }
  ...
  private void updateProperty( Session sess, String propertyName, String value,
      String description, String forServerOnly, Integer idCoreFacility,
      String codeRequestCategory ) {
    List props = generatePropertyQuery( sess, propertyName, idCoreFacility,
        codeRequestCategory ).list();
  ...
  private Query generatePropertyQuery( Session sess, String property,
      Integer idCoreFacility, String codeRequestCategory ) {
    Query propQuery = sess.createQuery(
        "SELECT p from PropertyDictionary p where p.propertyName = ? and p.idCoreFacility = ? and p.codeRequestCategory = ?" );
  ...
  private Boolean savePrepTypes( Session sess ) {
    if( prepTypesDoc == null
        || prepTypesDoc.getRootElement().getChildren().size() == 0 ) {
      return false;
    }
    ...
    int currentPrepTypeCount = sess.createQuery(
        "Select x from IsolationPrepType x" ).list().size();
    ...
    for( Iterator i = this.prepTypesDoc.getRootElement().getChildren().iterator(); i.hasNext(); ) {
      ...
      if( isNew != null && isNew.equals( "Y" ) ) {
        ...
        codeIsolationPrepType = IsolationPrepType.CODE_PREP_PREFIX
            + ++ currentPrepTypeCount;
      } else {
        ipt = ( IsolationPrepType ) sess.load( IsolationPrepType.class,
            node.getAttributeValue( "codeIsolationPrepType" ) );
        codeIsolationPrepType = node.getAttributeValue(
      ...
      ipt.setCodeRequestCategory(
          node.getAttributeValue( "codeRequestCategory" ) );
      prepTypes.add( ipt );
      sess.save( ipt );
      ...
      if( price == null ) {
        ...
        price.setIdPriceCategory(
            getDefaultIsolationPrepTypePriceCategoryId( sess, rc ) );
        ...
        price.setUnitPriceExternalAcademic( node.getAttributeValue(
            ...
            && ! node.getAttributeValue( "unitPriceExternalAcademic" ).equals(
                "" ) ? new BigDecimal(
                    node.getAttributeValue( "unitPriceExternalAcademic" ) )
                    : BigDecimal.ZERO );
        price.setUnitPriceExternalCommercial( node.getAttributeValue(
            "unitPriceExternalCommercial" ) != null
            && ! node.getAttributeValue( "unitPriceExternalCommercial" ).equals(
                "" ) ? new BigDecimal(
                    node.getAttributeValue( "unitPriceExternalCommercial" ) )
                    : BigDecimal.ZERO );
        sess.save( price );
        sess.flush();
    ...
    sess.flush();
    List existingPrepTypes = sess.createQuery(
        "SELECT x from IsolationPrepType x" ).list();
    ...
    for( Iterator j = existingPrepTypes.iterator(); j.hasNext(); ) {
      ...
      Price existingPrice = IsolationPrepType.getIsolationPrepTypePrice( sess,
          ep );

      if( ! prepTypes.contains( ep ) ) {
        if( sess.createQuery(
            "Select r from Request r where codeIsolationPrepType = '"
                + ep.getCodeIsolationPrepType() + "'" ).list().size() != 0 ) {
          ep.setIsActive( "N" );
          ...
          if( getPriceBillingItems( existingPrice, sess ) == null
              || getPriceBillingItems( existingPrice, sess ).size() == 0 ) {
  ...
  private List getPriceBillingItems( Price price, Session sess ) {
    ...
    String billingItemQuery = "SELECT bi from BillingItem as bi where bi.idPrice="
        + price.getIdPrice();
  ...
  private void saveSampleTypes( Session sess, RequestCategory rc ) {
    if( sampleTypesDoc == null
        || sampleTypesDoc.getRootElement().getChildren().size() == 0 ) {
      return;
    }
    //
    // Save sampleTypes
    ...
    for( Iterator i = this.sampleTypesDoc.getRootElement().getChildren().iterator(); i.hasNext(); ) {
      ...
      if( id.startsWith( "SampleType" ) ) {
        ...
        st = ( SampleType ) sess.load( SampleType.class,
            Integer.valueOf( id ) );
      }

      st.setSampleType( node.getAttributeValue( "display" ) );
      st.setIsActive( node.getAttributeValue( "isActive" ) );
      st.setCodeNucleotideType(
          node.getAttributeValue( "codeNucleotideType" ) );
      st.setNotes( node.getAttributeValue( "notes" ) );
      st.setIdCoreFacility( rc.getIdCoreFacility() );
      if( node.getAttributeValue( "sortOrder" ) == null
          || node.getAttributeValue( "sortOrder" ).length() == 0 ) {
        st.setSortOrder( null );
      } else {
        try {
          st.setSortOrder(
              Integer.parseInt( node.getAttributeValue( "sortOrder" ) ) );
        } catch( NumberFormatException e ) {
          st.setSortOrder( null );
        }
      }
      ...
      //
      List existingAssociations = sess.createQuery(
          "SELECT x from SampleTypeRequestCategory x where idSampleType = "
              + st.getIdSampleType() + " and x.codeRequestCategory = '"
              + rc.getCodeRequestCategory() + "'" ).list();
    ...
    // Remove sample types
    Query stRemoveQuery = sess.createQuery(
        "SELECT st from SampleType st where idCoreFacility=:idCoreFacility" );
    ...
    for( Iterator i = stRemoveQuery.list().iterator(); i.hasNext(); ) {
      ...
      if( ! sampleTypeMap.containsKey( sampleType.getIdSampleType() ) ) {
        ...
        Integer count = 0;
        List samples = sess.createQuery(
            "select count(*) from Sample s where s.idSampleType = "
                + sampleType.getIdSampleType() ).list();
        if( samples.size() > 0 ) {
          count += ( Integer ) samples.get( 0 );
        }
        List categories = sess.createQuery(
            "select count(*) from SampleTypeRequestCategory r where r.idSampleType = "
                + sampleType.getIdSampleType()
                + " AND r.codeRequestCategory != '"
                + rc.getCodeRequestCategory() + "'" ).list();
        ...
        if( deleteSampleType ) {
          List catToDelete = sess.createQuery(
              "select r from SampleTypeRequestCategory r where r.idSampleType = "
                  + sampleType.getIdSampleType() ).list();
          ...
          if( sampleType.getIsActive() != null
              && ! sampleType.getIsActive().equals( "N" ) ) {
  ...
  private void saveApplications( Session sess, RequestCategory rc ) {
    if( applicationsDoc == null
        || applicationsDoc.getRootElement().getChildren().size() == 0 ) {
      return;
    }
    ...
    Map<String, Price> illuminaLibPrepPriceMap = getIlluminaLibPrepPriceMap(
        sess, rc );
    Integer idPriceCategoryDefault = getDefaultLibPrepPriceCategoryId( sess,
        rc );
    Map<String, Price> qcLibPrepPriceMap = getQCLibPrepPriceMap( sess, rc );
    Integer idQCPriceCategoryDefault = getDefaultQCLibPrepPriceCategoryId( sess,
        rc );
    Map<String, Price> sequenomPriceMap = this.getSequenomPriceMap( sess, rc );
    Integer idSequenomPriceCategoryDefault = getDefaultSequenomPriceCategoryId(
        sess, rc );
    //
    // Save applications
    //
    ...
    for( Iterator i = this.applicationsDoc.getRootElement().getChildren().iterator(); i.hasNext(); ) {
      ...
      if( app.getCodeApplicationType() == null ) {
        app.setCodeApplicationType(
            ApplicationType.getCodeApplicationType( rct ) );
      }

      String idLabelingProtocolDefault = node.getAttributeValue(
          "idLabelingProtocolDefault" );
      String idHybProtocolDefault = node.getAttributeValue(
          "idHybProtocolDefault" );
      String idScanProtocolDefault = node.getAttributeValue(
          "idScanProtocolDefault" );
      String idFeatureExtractionProtocolDefault = node.getAttributeValue(
          "idFeatureExtractionProtocolDefault" );
      String idApplicationTheme = node.getAttributeValue(
          "idApplicationTheme" );
      String sortOrder = node.getAttributeValue( "sortOrder" );
      String samplesPerBatch = node.getAttributeValue( "samplesPerBatch" );

      app.setHasCaptureLibDesign(
          node.getAttributeValue( "hasCaptureLibDesign" ) );
      ...
      if( app.getHasChipTypes() == null
          || ( ! app.getHasChipTypes().equals( "Y" )
              && ! app.getHasChipTypes().equals( "N" ) ) ) {
        app.setHasChipTypes( "N" );
      }
      app.setApplication( node.getAttributeValue( "display" ) );
      if( app.getIsActive() != null && app.getIsActive().equals( "Y" )
          && ! node.getAttributeValue( "isActive" ).equals( "Y" ) ) {
        ...
        if( ! appSelectedInRequestCategory( sess, app,
            node.getAttributeValue( "isSelected" ), rc ) ) {
          app.setIsActive( node.getAttributeValue( "isActive" ) );
        }
      } else {
        app.setIsActive( node.getAttributeValue( "isActive" ) );
      ...
      app.setSortOrder( sortOrder != null && sortOrder.length() > 0
      ...
      if( this.requestCategoryApplicationsDoc == null ) {
        ...
        //
        List existingAssociations = sess.createQuery(
            "SELECT x from RequestCategoryApplication x where codeApplication = '"
                + app.getCodeApplication() + "' and x.codeRequestCategory = '"
                + rc.getCodeRequestCategory() + "'" ).list();
        if( node.getAttributeValue( "isSelected" ).equals( "Y" ) ) {
      ...
      List existingAssociations = sess.createQuery(
          "SELECT x from SeqLibProtocolApplication x where codeApplication = '"
              + app.getCodeApplication() + "'" ).list();
      ...
      for( Iterator i1 = existingProtocolMap.keySet().iterator(); i1.hasNext(); ) {
        ...
        if( ! protocolMap.containsKey( idSeqLibProtocol ) ) {
          SeqLibProtocolApplication x = existingProtocolMap.get(
              idSeqLibProtocol );
          sess.delete( x );
        }
      }
      ...
      saveIlluminaLibPrepPrices( sess, rc, app, node, illuminaLibPrepPriceMap,
          idPriceCategoryDefault );
      saveQCLibPrepPrices( sess, rc, app, null, node, qcLibPrepPriceMap,
          idQCPriceCategoryDefault );
      saveQCChipTypes( sess, rc, app, node, qcLibPrepPriceMap,
          idQCPriceCategoryDefault );
      saveSequenomPrices( sess, rc, app, node, sequenomPriceMap,
          idSequenomPriceCategoryDefault );
    ...
    for( Iterator i = appRemoveQuery.list().iterator(); i.hasNext(); ) {
      ...
      if( application.isApplicableApplication( rct, rc.getIdCoreFacility() )
          && ! applicationMap.containsKey(
              application.getCodeApplication() ) ) {

        boolean deleteApplication = true;
        List experiments = sess.createQuery(
            "select count(*)from Request r where r.codeApplication = '"
                + application.getCodeApplication() + "'" ).list();
        ...
        // Remove associations
        List existingsAssociations = sess.createQuery(
            "SELECT x from SeqLibProtocolApplication x where codeApplication = '"
                + application.getCodeApplication() + "'" ).list();
        ...
        existingsAssociations = sess.createQuery(
            "SELECT x from RequestCategoryApplication x where codeApplication = '"
                + application.getCodeApplication() + "'" ).list();
        for( Iterator i1 = existingsAssociations.iterator(); i1.hasNext(); ) {
        ...
        existingsAssociations = sess.createQuery(
            "SELECT x from BioanalyzerChipType x where codeApplication = '"
                + application.getCodeApplication() + "'" ).list();
        if( existingsAssociations.size() > 0 ) {
          for( Iterator i1 = existingsAssociations.iterator(); i1.hasNext(); ) {
            ...
            // set it to inactive.
            Price p1 = illuminaLibPrepPriceMap.get(
                application.getCodeApplication() );
            Price p2 = qcLibPrepPriceMap.get( application.getCodeApplication()
                + "&" + ( x != null ? x.getCodeBioanalyzerChipType() : "" ) );
          ...
          Price p1 = illuminaLibPrepPriceMap.get(
              application.getCodeApplication() );
          Price p2 = qcLibPrepPriceMap.get(
              application.getCodeApplication() + "&" );
          Price p3 = sequenomPriceMap.get( application.getCodeApplication() );
          deleteOrInactivatePrice( deleteApplication, sess, p1, p2, p3 );
        }
  ...
  private void addDefaultProtocol( Session sess, Application app,
      HashMap<Integer, SeqLibProtocol> protocolMap ) {
  ...
  private void saveOligoBarcodeSchemesAllowed( Session sess, RequestCategory rc,
      Application app, Element node,
      HashMap<Integer, SeqLibProtocol> protocolMap ) {
    // Only save barcode schemes for illumina request categories.
    if( ! RequestCategory.isIlluminaRequestCategory(
        rc.getCodeRequestCategory() )
        && ! RequestCategory.isSequenom( rc.getCodeRequestCategory() ) ) {
      return;
    }
    ...
    if( protocolMap.keySet().size() == 0 ) {
      if( idA != null || idB != null ) {
        log.error(
            "Unable to save oligo barcode scheme mapping for application "
                + app.getApplication()
                + " because no SeqLibProtocol is available." );
        return;
      }
    }
    ...
    for( OligoBarcodeSchemeAllowed obsa : existingSchemes ) {
      ...
      if( idB != null && obsa.getIdOligoBarcodeScheme().equals( idB )
          && obsa.getIsIndexGroupB() != null
          && obsa.getIsIndexGroupB().equals( "Y" ) ) {
        bFound = true;
        delete = false;
    ...
    if( ! aFound && idA != null ) {
      ...
      obsaA.setIdSeqLibProtocol(
          ( Integer ) protocolMap.keySet().toArray()[0] );
  ...
  private Integer getIdOligoBarcodeScheme( Element node, String aOrB ) {
    ...
    if( idAsString != null && idAsString.length() > 0 ) {
      try {
        ...
        log.error( "Unable to parse oligo barcode scheme " + aOrB + " for app "
            + node.getAttributeValue( "application" ) );
      }
    }
  ...
  private void saveQCChipTypes( Session sess, RequestCategory rc,
      Application app, Element node, Map<String, Price> qcLibPrepPriceMap,
      Integer idQCPriceCategoryDefault ) {
    if( ! RequestCategory.isQCRequestCategory( rc.getCodeRequestCategory() )
        || ! app.getHasChipTypes().equals( "Y" )
        || ! app.getIsActive().equals( "Y" ) ) {
      return;
    }
    //
    ...
    for( Iterator appIter = node.getChildren().iterator(); appIter.hasNext(); ) {
      ...
      if( appChild.getName() == "ChipTypes" ) {
        for( Iterator chipIter = appChild.getChildren().iterator(); chipIter.hasNext(); ) {
          ...
          BioanalyzerChipType chip = existMap.get(
              chipNode.getAttributeValue( "codeBioanalyzerChipType" ) );
          if( chip == null ) {
            chip = new BioanalyzerChipType();
            chip.setCodeBioanalyzerChipType(
                getNextUnassignedCodeBioanalyzerChipType( sess ) );
          } else {
            foundChipTypes.put( chip.getCodeBioanalyzerChipType(), "" );
          }
          setBioanalyzerChipType( chip, app, chipNode );
          sess.save( chip );
          saveQCLibPrepPrices( sess, rc, app, chip, chipNode, qcLibPrepPriceMap,
              idQCPriceCategoryDefault );
          sess.flush();
        }
      }
    ...
    for( Iterator existIter = existingChipTypes.iterator(); existIter.hasNext(); ) {
      BioanalyzerChipType chipType = ( BioanalyzerChipType ) existIter.next();
      if( foundChipTypes.get( chipType.getCodeBioanalyzerChipType() ) == null
          && chipType.getIsActive().equals( "Y" ) ) {
  ...
  private void setBioanalyzerChipType( BioanalyzerChipType chip,
      Application app, Element chipNode ) {
    chip.setIsActive( "Y" );
    chip.setCodeApplication( app.getCodeApplication() );
    chip.setBioanalyzerChipType(
        chipNode.getAttributeValue( "bioanalyzerChipType" ) );
    chip.setConcentrationRange(
        chipNode.getAttributeValue( "concentrationRange" ) );
    chip.setMaxSampleBufferStrength(
        chipNode.getAttributeValue( "maxSampleBufferStrength" ) );
    chip.setProtocolDescription(
    ...
    if( chipNode.getAttributeValue( "sampleWellsPerChip" ) != null ) {
      try {
        Integer wells = Integer.parseInt(
            chipNode.getAttributeValue( "sampleWellsPerChip" ) );
  ...
  private void removeChipType( BioanalyzerChipType chipType, Session sess ) {
    ...
    Query q = sess.createQuery(
        "Select codeBioanalyzerChipType from Request where codeBioanalyzerChipType=:code" );
    ...
    if( ! used ) {
      q = sess.createQuery(
          "Select codeBioanalyzerChipType from Sample where codeBioanalyzerChipType=:code" );
      q.setParameter( "code", chipType.getCodeBioanalyzerChipType() );
      if( q.list().size() > 0 ) {
        used = true;
      }
    ...
    if( ! used ) {
      q = sess.createQuery(
          "Select codeBioanalyzerChipType from SubmissionInstruction where codeBioanalyzerChipType=:code" );
      q.setParameter( "code", chipType.getCodeBioanalyzerChipType() );
      if( q.list().size() > 0 ) {
        used = true;
      }
  ...
  private Map<String, Price> getIlluminaLibPrepPriceMap( Session sess,
      RequestCategory rc ) {
    ...
    String queryString = "select p, crit " + " from PriceSheet ps "
        + " join ps.requestCategories rc " + " join ps.priceCategories pc "
        + " join pc.priceCategory.prices p " + " join p.priceCriterias crit "
  ...
  private Map<String, Price> getSequenomPriceMap( Session sess,
      RequestCategory rc ) {
    if( ! hasPriceSheet( sess, rc ) ) {
      return null;
    }
  ...
  private Map<String, Price> getQCLibPrepPriceMap( Session sess,
      RequestCategory rc ) {
    if( ! hasPriceSheet( sess, rc ) ) {
      return null;
    }
    ...
    for( Iterator i = l.iterator(); i.hasNext(); ) {
      ...
      String key = priceCriteria.getFilter1() + "&"
          + ( priceCriteria.getFilter2() != null ? priceCriteria.getFilter2()
              : "" );
      map.put( key, price );
    }
  ...
  private Boolean hasPriceSheet( Session sess, RequestCategory rc ) {
    String queryString = "select rc " + " from PriceSheet ps "
  ...
  private Integer getDefaultLibPrepPriceCategoryId( Session sess,
      RequestCategory rc ) {
    Integer id = null;
    String catName = PropertyDictionaryHelper.getInstance(
        sess ).getCoreFacilityRequestCategoryProperty( rc.getIdCoreFacility(),
            rc.getCodeRequestCategory(),
            PropertyDictionary.ILLUMINA_LIBPREP_DEFAULT_PRICE_CATEGORY );
    if( catName == null ) {
      id = null;
    } else {
      String queryString = "select pc " + " from PriceSheet ps "
          + " join ps.requestCategories rc " + " join ps.priceCategories pspc "
  ...
  private Integer getDefaultSequenomPriceCategoryId( Session sess,
      RequestCategory rc ) {
    Integer id = null;
    String catName = PropertyDictionaryHelper.getInstance(
        sess ).getCoreFacilityRequestCategoryProperty( rc.getIdCoreFacility(),
            rc.getCodeRequestCategory(),
            PropertyDictionary.SEQUENOM_DEFAULT_PRICE_CATEGORY );
    if( catName == null ) {
      id = null;
    } else {
      String queryString = "select pc " + " from PriceSheet ps "
  ...
  private void saveIlluminaLibPrepPrices( Session sess, RequestCategory rc,
      Application app, Element node, Map<String, Price> map,
      Integer defaultCategoryId ) {
    // Only save lib prep prices for illumina request categories that have price
    // sheet defined.
    if( map == null || ! RequestCategory.isIlluminaRequestCategory(
        rc.getCodeRequestCategory() ) ) {
      return;
    }
    ...
    if( price == null ) {
      if( defaultCategoryId == null ) {
        ...
        log.error(
            "SaveExperimentPlatform: Unable to store new lib prep price due to no default category for "
                + rc.getCodeRequestCategory() );
        return;
      }
      price = new Price();
    ...
    if( ( internal.length() == 0 && academic.length() == 0
        && commercial.length() == 0 )
        || ( internal.equals( "0.00" ) && academic.equals( "0.00" )
            && commercial.equals( "0.00" ) ) ) {
      return;
    }
  ...
  private void saveSequenomPrices( Session sess, RequestCategory rc,
      Application app, Element node, Map<String, Price> map,
      Integer defaultCategoryId ) {
    ...
    if( price == null ) {
      if( defaultCategoryId == null ) {
        // no default price category -- can't store new price.
        log.error(
            "SaveExperimentPlatform: Unable to store new sequenom price due to no default category for "
                + rc.getCodeRequestCategory() );
        return;
      }
      price = new Price();
  ...
  private Boolean priceModified( Element node ) {
    if( node.getAttributeValue( "unitPriceInternal" ) != null
        && node.getAttributeValue( "unitPriceInternal" ).length() > 0 ) {
      return true;
    }
    if( node.getAttributeValue( "unitPriceExternalAcademic" ) != null
        && node.getAttributeValue(
            "unitPriceExternalAcademic" ).length() > 0 ) {
      return true;
    }
    if( node.getAttributeValue( "unitPriceExternalCommercial" ) != null
        && node.getAttributeValue(
  ...
  private Integer getDefaultQCLibPrepPriceCategoryId( Session sess,
      RequestCategory rc ) {
    Integer id = null;
    String queryString = "select pc " + " from PriceSheet ps "
        + " join ps.requestCategories rc " + " join ps.priceCategories pspc "
  ...
  private void saveQCLibPrepPrices( Session sess, RequestCategory rc,
      Application app, BioanalyzerChipType chipType, Element node,
      Map<String, Price> map, Integer defaultCategoryId ) {
    // Only save lib prep prices for qc request categories that have price sheet
    // defined.
    if( ! RequestCategory.isQCRequestCategory( rc.getCodeRequestCategory() )
        || map == null || ! priceModified( node ) ) {
      return;
    }

    Boolean newPrice = false;
    Price price = map.get( app.getCodeApplication() + "&"
        + ( chipType != null ? chipType.getCodeBioanalyzerChipType() : "" ) );
    if( price == null ) {
      if( defaultCategoryId == null ) {
        // no default price category -- can't store new price.
        log.error(
    ...
    if( newPrice ) {
      ...
      sess.save( crit );
      sess.flush();
      map.put( app.getCodeApplication() + "&"
          + ( chipType != null ? chipType.getCodeBioanalyzerChipType() : "" ),
          price );
    }
  }
  ...
  private void saveRequestCategoryApplications( Session sess ) {
    if( requestCategoryApplicationsDoc == null
        || requestCategoryApplicationsDoc.getRootElement().getChildren().size() == 0 ) {
      return;
    }
    ...
    for( Iterator i = this.requestCategoryApplicationsDoc.getRootElement().getChildren().iterator(); i.hasNext(); ) {
      Element node = ( Element ) i.next();
      String key = node.getAttributeValue( "codeApplication" ) + "\t"
          + node.getAttributeValue( "codeRequestCategory" );
      node.setAttribute( "isStored", "N" );
      requestCategoryApplicationMap.put( key, node );
    }

    for( RequestCategoryApplication recApp : ( List<RequestCategoryApplication> ) sess.createQuery(
        "from RequestCategoryApplication" ).list() ) {
      String key = recApp.getCodeApplication() + "\t"
          + recApp.getCodeRequestCategory();
    ...
    for( String key : requestCategoryApplicationMap.keySet() ) {
      Element node = requestCategoryApplicationMap.get( key );
      if( ! node.getAttributeValue( "isStored" ).equals( "Y" )
          && node.getAttributeValue( "isSelected" ).equals( "Y" ) ) {
  ...
  private Boolean appSelectedInRequestCategory( Session sess, Application app,
      String isSelected, RequestCategory rc ) {
    Boolean hasSelections = false;
    if( isSelected.equals( "Y" ) ) {
      hasSelections = true;
    } else {
      Query q = sess.createQuery(
          "Select x from RequestCategoryApplication x where codeApplication = :codeApplication AND codeRequestCategory != :codeRequestCategory" );
  ...
  private void saveSequencingOptions( Session sess, RequestCategory rc ) {
    ...
    if( sequencingOptionsDoc == null
        || sequencingOptionsDoc.getRootElement().getChildren().size() == 0 ) {
      return;
    }
    ...
    Map<String, Price> illuminaSeqOptionPriceMap = getIlluminaSeqOptionPriceMap(
        sess, rc );
    Integer idPriceCategoryDefault = getDefaultSeqOptionPriceCategoryId( sess,
        rc );
    for( Iterator i = this.sequencingOptionsDoc.getRootElement().getChildren().iterator(); i.hasNext(); ) {
      Element node = ( Element ) i.next();
      NumberSequencingCyclesAllowed cyclesAllowed = null;

      String idNumberSequencingCyclesAllowed = node.getAttributeValue(
          "idNumberSequencingCyclesAllowed" );

      // Save new numberSequencingCycles or load the existing one
      if( idNumberSequencingCyclesAllowed.startsWith(
          "NumberSequencingCyclesAllowed" ) ) {
        cyclesAllowed = new NumberSequencingCyclesAllowed();
      } else {
        cyclesAllowed = ( NumberSequencingCyclesAllowed ) sess.load(
            NumberSequencingCyclesAllowed.class,
            Integer.valueOf( idNumberSequencingCyclesAllowed ) );
      }

      cyclesAllowed.setCodeRequestCategory( rc.getCodeRequestCategory() );
      cyclesAllowed.setIdNumberSequencingCycles( Integer.valueOf(
          node.getAttributeValue( "idNumberSequencingCycles" ) ) );
      cyclesAllowed.setIdSeqRunType(
          Integer.valueOf( node.getAttributeValue( "idSeqRunType" ) ) );
      ...
      cyclesAllowed.setProtocolDescription(
          node.getAttributeValue( "protocolDescription" ) );
      sess.save( cyclesAllowed );

      numberSequencingCyclesAllowedMap.put(
          cyclesAllowed.getIdNumberSequencingCyclesAllowed(),
          cyclesAllowed.getIdNumberSequencingCyclesAllowed() );

      saveIlluminaSeqOptionPrices( sess, rc, cyclesAllowed, node,
          illuminaSeqOptionPriceMap, idPriceCategoryDefault );
    }
    sess.flush();
    ...
    allCyclesAllowedQuery.setParameter( "codeRequestCategory",
        rc.getCodeRequestCategory() );
    for( Iterator i = allCyclesAllowedQuery.list().iterator(); i.hasNext(); ) {
      ...
      if( ! numberSequencingCyclesAllowedMap.containsKey(
          numberSequencingCyclesAllowed.getIdNumberSequencingCyclesAllowed() ) ) {
  ...
  private Integer getDefaultSeqOptionPriceCategoryId( Session sess,
      RequestCategory rc ) {
    String catName = PropertyDictionaryHelper.getInstance(
        sess ).getCoreFacilityRequestCategoryProperty( rc.getIdCoreFacility(),
            rc.getCodeRequestCategory(),
            PropertyDictionary.ILLUMINA_SEQOPTION_DEFAULT_PRICE_CATEGORY );
    Integer id = null;
    if( catName == null ) {
      id = null;
    } else {
  ...
  private Integer getDefaultIsolationPrepTypePriceCategoryId( Session sess,
      RequestCategory rc ) {
    String catName = PropertyDictionaryHelper.getInstance(
        sess ).getCoreFacilityRequestCategoryProperty( rc.getIdCoreFacility(),
            rc.getCodeRequestCategory(),
            PropertyDictionary.ISOLATION_DEFAULT_PRICE_CATEGORY );
    Integer id = null;
    if( catName == null ) {
      id = null;
    } else {
  ...
  private Map<String, Price> getIlluminaSeqOptionPriceMap( Session sess,
      RequestCategory rc ) {
    if( ! hasPriceSheet( sess, rc ) ) {
      return null;
    }
  ...
  private void saveIlluminaSeqOptionPrices( Session sess, RequestCategory rc,
      NumberSequencingCyclesAllowed cyclesAllowed, Element node,
      Map<String, Price> map, Integer defaultCategoryId ) {
    // Only save lib prep prices for illumina request categories that have price
    // sheet defined.
    if( ! RequestCategory.isIlluminaRequestCategory(
        rc.getCodeRequestCategory() ) || map == null
        || ! priceModified( node ) ) {
      return;
    }
    ...
    Price price = map.get(
        cyclesAllowed.getIdNumberSequencingCyclesAllowed().toString() );
    if( price == null ) {
      if( defaultCategoryId == null ) {
        // no default price category -- can't store new price.
        log.error(
            "SaveExperimentPlatform: Unable to store new seq option price due to no default category for "
                + rc.getCodeRequestCategory() );
        return;
      }
      price = new Price();
  ...
  private Integer getNextAssignedAppNumber( Session sess ) {
    int lastNumber = 0;
    List apps = sess.createQuery(
        "SELECT a.codeApplication from Application a where a.codeApplication like 'APP%'" ).list();
  ...
  private String getNextUnassignedCodeBioanalyzerChipType( Session sess ) {
    ...
    List apps = sess.createQuery(
        "SELECT b.codeBioanalyzerChipType from BioanalyzerChipType b where b.codeBioanalyzerChipType like 'BCT%'" ).list();
    for( Iterator i = apps.iterator(); i.hasNext(); ) {
  ...
  private Integer getNextAssignedRequestCategoryNumber( Session sess ) {
    int lastNumber = 0;
    List requestCategories = sess.createQuery(
        "SELECT rc.codeRequestCategory from RequestCategory rc where rc.codeRequestCategory like 'EXP%'" ).list();
  ...
  private void deleteOrInactivatePrice( Boolean deleteApplication, Session sess,
      Price p1, Price p2, Price p3 ) {
    if( deleteApplication ) {
      if( p1 != null ) {
        if( getPriceBillingItems( p1, sess ) == null
            || getPriceBillingItems( p1, sess ).size() == 0 ) {
          sess.delete( p1 );
        } else {
          p1.setIsActive( "N" );
          sess.save( p1 );
      ...
      if( p2 != null ) {
        if( getPriceBillingItems( p2, sess ) == null
            || getPriceBillingItems( p2, sess ).size() == 0 ) {
          sess.delete( p2 );
        } else {
          p2.setIsActive( "N" );
          sess.save( p2 );
      ...
      if( p3 != null ) {
        if( getPriceBillingItems( p3, sess ) == null
            || getPriceBillingItems( p3, sess ).size() == 0 ) {
          sess.delete( p3 );
        } else {
          p3.setIsActive( "N" );
          sess.save( p3 );
