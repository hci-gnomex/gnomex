    ...
    try{
      ...
      for(Object temp : nsca)
      {
        ...
        StringBuffer query1 = new StringBuffer("select seqLane.number, flowCell.lastCycleDate, " +
            ...
            "left join r.appUser as user " +
            "left join r.sequenceLanes as seqLane " +
      ...
      for(String temp : listItems)
      {
        ...
        //Start for loop here  for(Dictionary Entry : numbersequencingcyclesAllowed){
        StringBuffer query2 = new StringBuffer("select seqLane.number, " +
            "flowCell.lastCycleDate, " +
            "user.lastName, user.firstName, seqLane.idFlowCellChannel, flowCell.pipelineDate, " +
            "samp.qualDate, samp.seqPrepByCore, samp.seqPrepDate, flowCell.clustersPerTile, flowCell.pipelineFailed, flowCell.firstCycleDate, flowCell.pipelineDate " +
            "from Request as r " +
            "left join r.appUser as user " +
            "left join r.sequenceLanes as seqLane " +
            "left join seqLane.flowCellChannel as flowCell "+
            "left join seqLane.sample as samp "+
            "where (flowCell.pipelineDate is NULL or " +
            "r.completedDate is NULL) and " +
            "seqLane.idSeqRunType = " + cycle.getIdSeqRunType() + " and seqLane.idNumberSequencingCycles = " + cycle.getIdNumberSequencingCycles() +
            "and flowCell.lastCycleFailed = 'N' and flowCell.firstCycleFailed = 'N' and flowCell.pipelineFailed = 'N' "+
        "Order by flowCell.lastCycleDate asc");

//        StringBuffer status = new StringBuffer("select samp.qualDate, samp.seqPrepByCore, samp.seqPrepDate, seqLane.idFlowCellChannel, "+
//            "flow.pipelineDate, seqLane.number, flow.clustersPerTile "+
//            "from Sample as samp, SequenceLane as seqLane, FlowCellChannel as flow, Request as r "+
//            "where samp.idSample = seqLane.idSample and " +
//            "seqLane.idFlowCellChannel = flow.idFlowCellChannel and "+
//            "samp.idRequest = r.idRequest and "+
//            "r.completedDate is NULL or "+
//            "flowCell.pipelineDate is NULL) and "+
//            "flow.lastCycleFailed = 'N' and "+
//            "flow.pipelineFailed = 'N' and "+
//        "flow.firstCycleFailed = 'N'");

        List<Object[]> sqlResults = sess.createQuery(query2.toString()).list();

        //statusResults = sess.createQuery(status.toString()).list();
  ...
  private String getStatus(Object[] temp){
      if(temp[6] == null){// if sample quality date is null
        return "Sample Quality";
      }
      else if(temp[7] == null){ //if seqPrepByCore is null
        return "Library Prep Stage";
      }
      else if(temp[11] == null){ // if first cycle date is null
        return "Cluster Generation";
      }
      else if(temp[12] == null){ // if data pipeline date is null
        return "Sequence Run";
      }
      else
        return "Data Pipeline Stage";
  }
