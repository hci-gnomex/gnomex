...
public class ReportIssueFeedbackServlet extends HttpServlet {
	 ...
	 * a new security advisor if one is not found, the Safari browser cannot handle authenticating the user (this second time). So for now, this servlet must be
	 * run non-secure.
	 */
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		try {
			...
			if (!MailUtil.isValidEmail(fromAddress)) {
				...
				emailBody = "Time: " + sdf.format(currentDate) + "\r" + "RequestURL: " + req.getRequestURL().toString() + "\r" + "AppUserName: " + "Anonymous "
						+ "\r" + "-------------------------------------------User Feedback-----------------------------------------------------" + "\r"
						+ body.toString();
			} else {

				emailBody = "Time: " + sdf.format(currentDate) + "\r" + "RequestURL: " + req.getRequestURL().toString() + "\r" + "IdAppUser: " + this.IdAppUser
						+ "\r" + "AppUserName: " + this.AppUserName + "\r" + "UNID: " + this.UNID + "\r"
						+ "-------------------------------------------User Feedback-----------------------------------------------------" + "\r"
						+ body.toString();
			}

			MailUtilHelper helper = new MailUtilHelper(emailRecipients, fromAddress, theSubject, emailBody, outputfile, format.equalsIgnoreCase("HTML") ? true
					: false, dh, serverName);
