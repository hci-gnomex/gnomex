...
public class SaveProductLedgerEntryList extends GNomExCommand implements Serializable {
	...
	public void loadCommand(HttpServletRequest request, HttpSession session) {
		...
		if (request.getParameter("productLedgerEntryList") != null && !request.getParameter("productLedgerEntryList").equals("")) {
		...
		try {
			if (this.isValid()) {
				...
				for (Element entry : ledgerEntries) {
					ProductLedger productLedger = sess.load(ProductLedger.class, Integer.parseInt(entry.getAttributeValue("idProductLedger")));
