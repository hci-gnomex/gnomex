...
public class GetRequestDownloadList extends GNomExCommand implements Serializable {
				...
				if (isSolexaRequest) {
					...
					if (flowCellNumbers != null) {
						for (Iterator i1 = flowCellNumbers.iterator(); i1.hasNext();) {
							...
							String fcKey = theCreateYear + Constants.DOWNLOAD_KEY_SEPARATOR + sortDate + Constants.DOWNLOAD_KEY_SEPARATOR
									+ fcFolder.getRequestNumber() + Constants.DOWNLOAD_KEY_SEPARATOR + fcFolder.getFlowCellNumber()
									+ Constants.DOWNLOAD_KEY_SEPARATOR + fcFolder.getIdCoreFacility() + Constants.DOWNLOAD_KEY_SEPARATOR
	...
	public static void addExpandedFileNodes(Session sess, String serverName, String baseDirFlowCell, Element requestNode, Element requestDownloadNode,
			String requestNumber, String key, String codeRequestCategory, DictionaryHelper dh, boolean isFlowCellDirectory) throws XMLReflectException {
		...
		UploadDownloadHelper.getFileNamesToDownload(sess, serverName, baseDirFlowCell, key, requestNumbers, requestMap, directoryMap,
				dh.getPropertyDictionary(PropertyDictionary.FLOWCELL_DIRECTORY_FLAG));
	...
	private static String getLinkedSampleNumber(Session sess, String fileName) {
		...
		if (expFile.size() > 0) {
			...
			List sampleNumber = sess.createQuery(
					"Select samp.number from SampleExperimentFile sef JOIN sef.sample as samp where sef.idExpFileRead1 = " + ef.getIdExperimentFile()
							+ " or sef.idExpFileRead2 = " + ef.getIdExperimentFile()).list();
	...
	private void addRootFileNodes(String baseDir, Element requestNode, String requestNumber, String createDate, String subDirectory, Session sess)
			throws Exception {
