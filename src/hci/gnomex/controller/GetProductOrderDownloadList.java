...
public void loadCommand(HttpServletRequest req, HttpSession sess) {
	...
	if (req.getParameter("skipUploadStagingDirFiles") != null
			&& !req.getParameter("skipUploadStagingDirFiles").equals("")) {
			...
			for (Iterator i = productOrderNumbers.iterator(); i.hasNext();) {
				...
				for (Iterator i1 = directoryKeys.iterator(); i1.hasNext();) {
					...
					if (includeUploadStagingDir.equals("Y")) {
						...
						addExpandedFileNodes(baseDir, poNode, poNode, productOrderNumber, key, dh,
								knownProductOrderFileMap, fileMap, sess);
					} else {
						...
						addExpandedFileNodes(baseDir, poNode, dummyNode, productOrderNumber, key, dh,
								knownProductOrderFileMap, fileMap, sess);
					}
					...
					for (Iterator i2 = theFiles.iterator(); i2.hasNext();) {
						...
						if (fd.getDisplayName().equals(Constants.UPLOAD_STAGING_DIR)
								&& skipUploadStagingDirFiles.equals("Y")) {
							continue;
						}

						ProductOrderFile pof = (ProductOrderFile) knownProductOrderFileMap.get(fd
						...
						String viewType = Constants.DOWNLOAD_PRODUCT_ORDER_SINGLE_FILE_SERVLET + "?idProductOrder="
						...
						fdNode.setAttribute("idProductOrder", po.getIdProductOrder() != null ? po.getIdProductOrder()
								.toString() : "");
						fdNode.setAttribute("dirty", "N");
						fdNode.setAttribute(
								"key",
								directoryName != null && directoryName.length() > 0 ? po.getKey(directoryName) : po
										.getKey());
						...
						fdNode.setAttribute("qualifiedFilePath",
								fd.getQualifiedFilePath() != null ? fd.getQualifiedFilePath() : "");
						fdNode.setAttribute("baseFilePath", fd.getBaseFilePath() != null ? fd.getBaseFilePath() : "");
						fdNode.setAttribute("comments", comments);
						fdNode.setAttribute("lastModifyDate", fd.getLastModifyDate() != null ? fd.getLastModifyDate()
								.toString() : "");
...
public static void addExpandedFileNodes(String baseDir, Element poNode, Element poDownloadNode,
		String productOrderNumber, String key, DictionaryHelper dh, Map knownProductOrderFileMap, Map fileMap,
	...
	GetProductOrderDownloadList.getFileNamesToDownload(baseDir, key, productOrderIds, productOrderMap, directoryMap,
			true);
	List directoryKeys = (List) productOrderMap.get(productOrderNumber);
	...
	for (Iterator i1 = directoryKeys.iterator(); i1.hasNext();) {
		...
		if (theFiles != null && theFiles.size() > 0) {
			for (Iterator i2 = theFiles.iterator(); i2.hasNext();) {
				...
				fd.setQualifiedFilePath(directoryName);
				fd.setBaseFilePath(baseDir + createYear + "/" + productOrderNumber);
				fd.setId(poNode.getAttributeValue("idProductOrder") != null ? Integer.valueOf(poNode
						.getAttributeValue("idProductOrder")) : null);
				fd.setNumber(poNode.getAttributeValue("productOrderNumber") != null ? poNode
						.getAttributeValue("productOrderNumber") : null);
				String idLab = poNode.getAttributeValue("idLab");
				fd.setIdLab(idLab == null || idLab.equals("") ? null : Integer.valueOf(idLab));
				fd.excludeMethodFromXML("getChildren");
...
private static void recurseAddChildren(Element fdNode, FileDescriptor fd, Map fileMap, Map knownFilesMap, Session sess)
	...
	for (Iterator i = fd.getChildren().iterator(); i.hasNext();) {
		...
		childFdNode.setAttribute("filePathName",
				childFd.getQualifiedFilePath() != null ? childFd.getQualifiedFilePath() : "");
		childFdNode.setAttribute("qualifiedFileName",
				childFd.getQualifiedFileName() != null ? childFd.getQualifiedFileName() : "");
		childFdNode.setAttribute("qualifiedFilePath",
				childFd.getQualifiedFilePath() != null ? childFd.getQualifiedFilePath() : "");
		...
		childFdNode.setAttribute("lastModifyDateDisplay",
				childFd.getLastModifyDateDisplay() != null ? childFd.getLastModifyDateDisplay() : "");
		childFdNode.setAttribute("uploadDate",
				childFd.getUploadDate() != null ? childFd.formatDate(childFd.getUploadDate(), DATE_OUTPUT_SQL) : "");
		...
		childFdNode.setAttribute("idProductOrderFileString",
...
public static void getFileNamesToDownload(String baseDir, String keysString, List productOrderNumbers,
		Map productOrderMap, Map directoryMap, boolean flattenSubDirs) {
...
public static void getFileNames(String productOrderNumber, String directoryName, List theFiles, String subDirName,
		String baseDir, boolean flattenSubDirs) {
