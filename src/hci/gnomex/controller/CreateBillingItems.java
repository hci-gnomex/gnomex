...
import hci.gnomex.utility.comparators.PropertyEntryComparator;
...
public class CreateBillingItems extends GNomExCommand implements Serializable {
  ...
  public void loadCommand( HttpServletRequest request, HttpSession session ) {
    ...
    if( request.getParameter( "idRequest" ) != null
        && ! request.getParameter( "idRequest" ).equals( "" ) ) {
      idRequest = new Integer( request.getParameter( "idRequest" ) );
    }

    if( request.getParameter( "requestXMLString" ) != null
        && ! request.getParameter( "requestXMLString" ).equals( "" ) ) {
    ...
    if( request.getParameter( "propertiesXML" ) != null
        && ! request.getParameter( "propertiesXML" ).equals( "" ) ) {
      propertiesXML = request.getParameter( "propertiesXML" );
    }

    if( request.getParameter( "idBillingPeriod" ) != null
        && ! request.getParameter( "idBillingPeriod" ).equals( "" ) ) {
      idBillingPeriod = new Integer(
          request.getParameter( "idBillingPeriod" ) );
    }

    if( idRequest == null && requestParser == null ) {
      this.addInvalidField( "idRequest",
          "idRequest or RequestXMLString is required." );
    }
    ...
    try {
      ...
      if( billingPeriod == null ) {
        throw new RollBackCommandException(
            "Cannot find current billing period in dictionary" );
      }
      ...
      if( idRequest != null ) {
        ...
        if( ! this.getSecAdvisor().hasPermission(
            SecurityAdvisor.CAN_MANAGE_BILLING ) ) {
          throw new RollBackCommandException(
              "Insufficient permission to create new billing items" );
        }
        ...
        // Clear session here so we don't get caught with an auto-flush later
        // on.
        sess.clear();
        ...
        if( ! this.getSecAdvisor().hasPermission(
            SecurityAdvisor.CAN_MANAGE_BILLING )
            && ! this.getSecAdvisor().isGroupIAmMemberOrManagerOf(
                request.getIdLab() )
            && ! this.getSecAdvisor().isLabICanSubmitTo( l )
            && ! this.getSecAdvisor().isGroupICollaborateWith(
                request.getIdLab() ) ) {
          throw new RollBackCommandException(
              "Insufficient permission to view estimated charges" );
        }
        ...
        if( request.getIdSlideProduct() != null ) {
          SlideProduct slideProduct = ( SlideProduct ) sess.load(
              SlideProduct.class, request.getIdSlideProduct() );
        ...
        if( ! requestParser.isAmendRequest()
            || requestParser.getAmendState().equals(
                Constants.AMEND_QC_TO_SEQ ) ) {
          for( Iterator i = requestParser.getSampleIds().iterator(); i.hasNext(); ) {
            String idSampleString = ( String ) i.next();
            Sample sample = ( Sample ) requestParser.getSampleMap().get(
                idSampleString );
        ...
        // Parse the hybs. Plugin just need a thinly initialized Hyb for count
        // purposes in the
        x = 0;
        for( Iterator i = requestParser.getHybInfos().iterator(); i.hasNext(); ) {
          ...
          if( hybInfo.getIdSampleChannel1String() != null
              && ! hybInfo.getIdSampleChannel1String().equals( "" )
              && ! hybInfo.getIdSampleChannel1String().equals( "0" ) ) {
            labeledSampleChannel1Map.put( hybInfo.getIdSampleChannel1String(),
                null );
          }
          if( hybInfo.getIdSampleChannel2String() != null
              && ! hybInfo.getIdSampleChannel2String().equals( "" )
              && ! hybInfo.getIdSampleChannel2String().equals( "0" ) ) {
            labeledSampleChannel2Map.put( hybInfo.getIdSampleChannel2String(),
                null );
          }
          hybs.add( hyb );
        }
        ...
        for( Iterator i = requestParser.getSequenceLaneInfos().iterator(); i.hasNext(); ) {
          ...
          if( isNewLane ) {
            if( lane.getIdSequenceLane() == null ) {
              ...
              lane.setIdNumberSequencingCycles(
                  laneInfo.getIdNumberSequencingCycles() );
              lane.setIdNumberSequencingCyclesAllowed(
                  laneInfo.getIdNumberSequencingCyclesAllowed() );
            }
            lane.setIdSeqRunType( laneInfo.getIdSeqRunType() );
            Sample sample = ( Sample ) requestParser.getSampleMap().get(
                laneInfo.getIdSampleString() );
      ...
      List priceSheets = sess.createQuery(
          "SELECT ps from PriceSheet as ps" ).list();
      for( Iterator i = priceSheets.iterator(); i.hasNext(); ) {
        ...
        for( Iterator i1 = ps.getRequestCategories().iterator(); i1.hasNext(); ) {
          ...
          if( requestCategory.getCodeRequestCategory().equals(
              request.getCodeRequestCategory() ) ) {
            priceSheet = ps;
            break;
          }
      ...
      if( priceSheet != null ) {
        ...
        for( Iterator i1 = priceSheet.getPriceCategories().iterator(); i1.hasNext(); ) {
          ...
          if( priceCategory.getIsActive() != null
              && priceCategory.getIsActive().equals( "N" ) ) {
            continue;
          }
          ...
          if( priceCategory.getPluginClassName() != null ) {
            try {
              plugin = ( BillingPlugin ) Class.forName(
                  priceCategory.getPluginClassName() ).newInstance();
              if( priceCategory.getPluginClassName().toLowerCase().indexOf(
                  "discount" ) != - 1 ) {
                isDiscount = true;
              }
            } catch( Exception e ) {
              log.error( "Unable to instantiate billing plugin "
                  + priceCategory.getPluginClassName() );
            }
          ...
          if( plugin != null ) {
            List billingItemsForCategory = plugin.constructBillingItems( sess,
                idRequest != null ? "" : requestParser.getAmendState(),
                billingPeriod, priceCategory, request, samples, labeledSamples,
                hybs, lanes,
                requestParser != null ? requestParser.getSampleAssays() : null,
                BillingStatus.PENDING, propertyEntries );
            if( isDiscount ) {
      ...
      requestNode.setAttribute( "idRequest",
          request.getIdRequest().toString() );
      requestNode.setAttribute( "idBillingAccount",
          request.getIdBillingAccount().toString() );
      ...
      requestNode.setAttribute( "submitter", request.getAppUser() != null
          ? request.getAppUser().getDisplayName() : "" );
      requestNode.setAttribute( "codeRequestCategory",
          request.getCodeRequestCategory() );
      requestNode.setAttribute( "billingLabName", request.getLabName() );
      requestNode.setAttribute( "billingAccountName",
          request.getBillingAccount() != null
              ? request.getBillingAccount().getAccountName() : "" );
      ...
      for( Iterator i = billingItems.iterator(); i.hasNext(); ) {
        ...
        Element billingItemNode = bi.toXMLDocument( null,
            this.DATE_OUTPUT_SQL ).getRootElement();
        if( bi.getInvoicePrice() != null ) {
          grandInvoicePrice = grandInvoicePrice.add( bi.getInvoicePrice() );
          billingItemNode.setAttribute( "invoicePrice",
              nf.format( bi.getInvoicePrice().doubleValue() ) );
        }
      ...
      for( Iterator i = discountBillingItems.iterator(); i.hasNext(); ) {
        ...
        if( bi.getUnitPrice() != null ) {
          BigDecimal invoicePrice = bi.getUnitPrice().multiply(
              grandInvoicePrice );
          bi.setUnitPrice( invoicePrice );
          bi.setInvoicePrice( invoicePrice );
          billingItemNode.setAttribute( "invoicePrice",
              nf.format( bi.getInvoicePrice().doubleValue() ) );
          billingItemNode.setAttribute( "unitPrice",
              nf.format( bi.getInvoicePrice().doubleValue() ) );
      ...
      buf.append(
          "SELECT sum(bi.invoicePrice) from BillingItem bi where bi.idBillingAccount = "
              + request.getIdBillingAccount() );
      ...
      BillingAccount billingAccount = ( BillingAccount ) sess.load(
          BillingAccount.class, request.getIdBillingAccount() );
      billingAccount.setTotalChargesToDate( totalChargesToDate );

      requestNode.setAttribute( "invoicePrice",
          NumberFormat.getCurrencyInstance().format(
              grandInvoicePrice.doubleValue() ) );
      requestNode.setAttribute( "exceedsBillingAccountBalance",
          billingAccount.getTotalDollarAmountRemaining() != null
              && billingAccount.getTotalDollarAmountRemaining().doubleValue() < 0
                  ? "Y" : "N" );
      requestNode.setAttribute( "exceededDollarAmount",
          billingAccount.getTotalDollarAmountRemaining() != null
              && billingAccount.getTotalDollarAmountRemaining().doubleValue() < 0
                  ? NumberFormat.getCurrencyInstance().format(
                      billingAccount.getTotalDollarAmountRemaining().abs() )
                  : "N" );
  ...
  private Set saveRequestProperties( Session sess, RequestParser requestParser )
      throws org.jdom.JDOMException {
    Set<PropertyEntry> propertyEntries = new TreeSet<PropertyEntry>(
        new PropertyEntryComparator() );
    // Delete properties
    if( propertiesXML != null && ! propertiesXML.equals( "" ) ) {
      ...
      for( Iterator< ? > i = propsDoc.getRootElement().getChildren().iterator(); i.hasNext(); ) {
        ...
        if( idPropertyEntry == null || idPropertyEntry.equals( "" ) ) {
          ...
          pe = PropertyEntry.class.cast( sess.get( PropertyEntry.class,
              Integer.valueOf( idPropertyEntry ) ) );
        }
        pe.setIdProperty(
            Integer.valueOf( node.getAttributeValue( "idProperty" ) ) );
