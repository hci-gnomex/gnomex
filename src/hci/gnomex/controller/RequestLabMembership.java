...
public class RequestLabMembership extends GNomExCommand implements Serializable {
  ...
  public Command execute() throws GNomExRollbackException {
    try {
      ...
      if (currentUser.getGuid() == null || (currentUser.getGuid() != null && currentUser.getGuidExpiration().before(new Date(System.currentTimeMillis())))) {
  ...
  private void sendRequestEmail(Lab requestedLab, AppUser appUser, Session sess) throws NamingException, MessagingException, IOException {
    ...
    String approveURL = url + "/" + Constants.APPROVE_LAB_MEMBERSHIP_SERVLET + "?idAppUser=" + appUser.getIdAppUser().intValue() + "&idLab=" + requestedLab.getIdLab() + "&guid=" + uuidStr;
    String deleteURL = url + "/" + Constants.APPROVE_LAB_MEMBERSHIP_SERVLET + "?idAppUser=" + appUser.getIdAppUser().intValue() + "&denyRequest=Y" + "&guid=" + uuidStr + "&idLab=" + requestedLab.getIdLab();
    ...
    introForAdmin.append("<a href='" + approveURL + "'>Click here</a> to approve their request.  GNomEx will automatically send an email to notify the user that they have been added to your lab.<br><br>");
    introForAdmin.append("<a href='" + deleteURL + "'>Click here</a> to deny their request.  GNomEx will automatically send an email to notify the user that their request has been denied.<br><br>");
