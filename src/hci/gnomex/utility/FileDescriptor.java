...
public String getViewURL(String viewType) {
	String viewURL = "";
	String dirParm = this.getDirectoryName() != null && !this.getDirectoryName().equals("") ? "&dir="
		...
		if (found) {
			...
			try {
				maxSize = Math.pow(2, 20)
						* Double.parseDouble(PropertyDictionaryHelper.getInstance(null).getProperty(
								PropertyDictionary.FILE_MAX_VIEWABLE_SIZE));
			} catch (Exception ex) {
			}
