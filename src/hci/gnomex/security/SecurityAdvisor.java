...
public class SecurityAdvisor extends DetailObject implements Serializable, hci.framework.security.SecurityAdvisor {
    ...
    private SecurityAdvisor(AppUser appUser, boolean isGNomExUniversityUser, boolean isGNomExExternalUser, boolean isUniversityOnlyUser, boolean isLabManager,
                            boolean canAccessBSTX, String BSTpersonId, Integer idCoreFacility, Map<Integer, Integer> submitMap) throws InvalidSecurityAdvisorException {
    ...
    public static SecurityAdvisor create(Session sess, String uid, Integer idCoreFacility) throws InvalidSecurityAdvisorException {
        ...
        securityAdvisor = new SecurityAdvisor(appUser, isGNomExUniversityUser, isGNomExExternalUser, isUniversityOnlyUser, isLabManager, canAccessBSTX,
                BSTpersonId, idCoreFacility, submitMap);
    ...
    public boolean canRead(DetailObject object) throws UnknownPermissionException {
        ...
        if (object instanceof Request) {
                ...
                if (!canRead) {
                    ...
                    if (!canRead) {
                        ...
                        // Request has membership + collaborator visibility
                            ...
                            if (isGroupIAmMemberOf(req.getIdLab()) || isGroupIManage(req.getLab()) || isGroupICollaborateWith(req.getIdLab())) {
                                canRead = true;
                            }
                        }
			...
			/*
             * If visibility checks failed then do a final check to make sure that this isn't a request that a collaborator submitted and is trying to view We
			 * don't handle collaborators in above cases b/c it would give them access to all requests from the lab they collaborate with
			 */
            if (!canRead) {
            ...
            // Normal gnomex users
                ...
                if (!canRead) {
                    ...
                    // Analysis has membership + collaborator visiblity
                        ...
                        if (isGroupIAmMemberOf(a.getIdLab()) || isGroupIManage(a.getLab()) || isGroupICollaborateWith(a.getIdLab())) {
                            canRead = true;
                        }
                    }
            ...
            // Normal gnomex users
                ...
                if (!canRead) {
                    ...
                    // DataTrack has membership + collaborator visiblity
                        ...
                        if (isGroupIAmMemberOf(dt.getIdLab()) || isGroupIManage(dt.getLab()) || isGroupICollaborateWith(dt.getIdLab())) {
                            canRead = true;
                        }
                    }
        ...
        //
            ...
            // GNomEx Users
                ...
                if (isGroupIAmMemberOf(proj.getIdLab()) || isGroupIManage(proj.getLab()) || isGroupICollaborateWith(proj.getIdLab())) {
                    canRead = true;
                } else {
        ...
        //
            ...
            // GNomEx Users
                ...
                if (isGroupIAmMemberOf(ag.getIdLab()) || isGroupIManage(ag.getLab()) || isGroupICollaborateWith(ag.getIdLab())) {
                    canRead = true;
                } else {
        ...
        //
            ...
            // GNomEx Users
                ...
                if (isGroupIAmMemberOf(dtf.getIdLab()) || isGroupIManage(dtf.getLab()) || isGroupICollaborateWith(dtf.getIdLab())) {
                    canRead = true;
                } else {
        ...
        //
            ...
            if (hasPermission(CAN_ACCESS_ANY_OBJECT)) {
                ...
                if (sp.hasPublicExperiments()) {
                    ...
                    if (sp.getIdLab() == null) {
                        canRead = true;
                    } else if (isGroupIAmMemberOf(sp.getIdLab()) || isGroupIManage(sp.getIdLab()) || isGroupICollaborateWith(sp.getIdLab())) {
            ...
            // Normal gnomex users
                ...
                if (!canRead) {
                    ...
                    // Topic has membership + collaborator visiblity
                        ...
                        if (isGroupIAmMemberOf(t.getIdLab()) || isGroupIManage(t.getLab()) || isGroupICollaborateWith(t.getIdLab())) {
                            canRead = true;
                        }
                    }
    ...
    public Map<Integer, Integer> getBSTXSecurityIdsToExclude(Session sess, DictionaryHelper dh, List rows, Integer idRequestIdx, Integer codeRequestCategoryIdx)
    ...
    public boolean canUpdate(DetailObject object) throws UnknownPermissionException {
        ...
        if (object instanceof Request) {
            ...
            // Admins - Can only update requests from core facility user manages
            else if (hasPermission(SecurityAdvisor.CAN_WRITE_ANY_OBJECT) || hasPermission(SecurityAdvisor.CAN_PARTICIPATE_IN_GROUPS)) {
            ...
            if (!canUpdate && hasPermission(SecurityAdvisor.CAN_SUBMIT_FOR_OTHER_CORES) && this.isLabICanSubmitTo(req.getLab())
                    && this.appUser.getIdAppUser().equals(req.getIdSubmitter())) {
                canUpdate = true;
            }
        }
            ...
            if (object instanceof Request) {
                ...
                // Admins - Can only update requests from core facility user manages
                    ...
                    if (pdh.getCoreFacilityRequestCategoryProperty(req.getIdCoreFacility(), req.getCodeRequestCategory(),
                            ...
                            && !(req.getCodeRequestStatus().equals(RequestStatus.SUBMITTED) || req.getCodeRequestStatus().equals(RequestStatus.NEW) || (req
                            .getCodeRequestStatus().equals(RequestStatus.PROCESSING) && !req.onReactionPlate()))) {
                        canUpdate = false;
                    }
                }
                // University GNomEx users
                    ...
                    canUpdate = canUpdate(req);
                    if (canUpdate
                            && pdh.getCoreFacilityRequestCategoryProperty(req.getIdCoreFacility(), req.getCodeRequestCategory(),
                            PropertyDictionary.NEW_REQUEST_SAVE_BEFORE_SUBMIT).equals("Y") && !req.getCodeRequestStatus().equals(RequestStatus.NEW)) {
                        canUpdate = false;
                    }
                }
            } else {
        ...
        if (object instanceof Request) {
            ...
            // Admin - can delete experiments from core facility user manages
                ...
                if (pdh.getCoreFacilityRequestCategoryProperty(r.getIdCoreFacility(), r.getCodeRequestCategory(),
                        PropertyDictionary.NEW_REQUEST_SAVE_BEFORE_SUBMIT).equals("Y")
                        && !r.getCodeRequestStatus().equals(RequestStatus.SUBMITTED) && !r.getCodeRequestStatus().equals(RequestStatus.NEW)) {
                    canDelete = false;
                }
            }
    ...
    public Boolean canDeleteSample(Request req) throws UnknownPermissionException {
        if ((pdh != null && pdh.getCoreFacilityRequestCategoryProperty(req.getIdCoreFacility(), req.getCodeRequestCategory(),
                PropertyDictionary.NEW_REQUEST_SAVE_BEFORE_SUBMIT).equals("Y")) || (req.getIsExternal() != null && req.getIsExternal().equals("Y"))) {
    ...
    public boolean isGroupIManage(Integer idLab) {
        ...
        if (hasPermission(CAN_ADMINISTER_ALL_CORE_FACILITIES)) {
            ...
            try {
                List labList = GetCoreFacilityLabList.getLabListForCore(this.getHibernateSession(this.getUsername()),
                        idCoreFacility);
                for (Iterator i = labList.iterator(); i.hasNext(); ) {
                    Lab lab = (Lab) i.next();
                    if (lab.getIdLab().equals(idLab)) {
                        isMyLab = true;
                        break;
    ...
    public boolean buildSpannedSecurityCriteria(StringBuffer queryBuf, String inheritedClassShortName, String classShortName, String collabClassShortName,
                                                boolean isFirstCriteria, String visibilityField, boolean scopeToGroup, String leftJoinExclusionCriteria, String labCoreFacilitiesName,
    ...
    public boolean buildSecurityCriteria(StringBuffer queryBuf, String classShortName, String collabClassShortName, boolean isFirstCriteria,
                                         boolean scopeToGroup, boolean hasCoreFacility) {
    ...
    public boolean buildSecurityCriteria(StringBuffer queryBuf, String classShortName, String collabClassShortName, boolean isFirstCriteria,
                                         boolean scopeToGroup, boolean hasCoreFacility, boolean checkSubmitter) {
    ...
    public boolean buildSecurityCriteria(StringBuffer queryBuf, String classShortName, String collabClassShortName, boolean isFirstCriteria,
                                         boolean scopeToGroup, boolean hasCoreFacility, boolean checkSubmitter, boolean includeVisCrit) {
        if (hasPermission(SecurityAdvisor.CAN_ADMINISTER_ALL_CORE_FACILITIES)) {
