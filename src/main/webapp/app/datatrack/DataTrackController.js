...
// start the world
			...
			// this is called when clicking on something in the tree
			...
			// this is called when searching in top nav search or within data track view search
				...
				if (!$scope.showColumnForm) {
					...
					for (var i = 0; i<$scope.datatrackTree.length; i++) {
						...
						for (var j = 0;j<$scope.datatrackTree[i].children.length;j++) {
							...
							if (sel != null && sel.type == "D" && sel.number == $scope.searchFilter.lookup) {
								$scope.datatrackSelected = angular.copy(sel);
								$scope.datatrackForm = $scope.datatrackDetail;
					...
					// we searched for a data track and if we got something
					// then show it.  This is a KLUDGE to get around the
					// max limit of data tracks returned.
					// With the max limit the code above will never find the data track
					// we searched for.
					if(foundIT == false){
						$scope.datatrackSelected = $scope.xmlDataTrack;
						$scope.datatrackForm = $scope.datatrackDetail;
					}

				} // end of if !$scope.showColumnForm

				//if we get to the end then try just pushing the xml from getDataTrack as the selected datatrack
				// possibility that the datatrack isn't in the returned list.
