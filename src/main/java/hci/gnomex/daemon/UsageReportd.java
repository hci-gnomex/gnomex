...
public class UsageReportd extends TimerTask {
    ...
    public int weeklyTotalGuestDownloads = 0;
    public int cumTotalGuestDownloads = 0;
    public int weeklyTotalGuests = 0;
    public int cumTotalGuests = 0;
    public int weeklyTotalVisits = 0;
    public int cumTotalVisits = 0;
    ...
    public String tsYear = null;
    public String tsMonth = null;
    public String tsDay = null;

    public String teYear = null;
    public String teMonth = null;
    public String teDay = null;

    public String theEndDate = null;

    public UsageReportd(String[] args) {
        int i = 0;
        while (i < args.length) {
            System.out.println("*** top of loop *** i: " + i + " args.length: " + args.length);
            ...
            if (args[i].equals("-tsyear")) {
                tsYear = args[i + 1];
                System.out.println("[usaged] tsYear: " + tsYear);
                i = i + 2;
                continue;
            }

            if (args[i].equals("-tsmonth")) {
                tsMonth = args[i + 1];
                System.out.println("[usaged] tsMonth: " + tsMonth);
                i = i + 2;
                continue;
            }

            if (args[i].equals("-tsday")) {
                tsDay = args[i + 1];
                System.out.println("[usaged] tsDay: " + tsDay);
                i = i + 2;
                continue;
            }

            if (args[i].equals("-teyear")) {
                teYear = args[i + 1];
                System.out.println("[usaged] teYear: " + teYear);
                i = i + 2;
                continue;
            }

            if (args[i].equals("-temonth")) {
                teMonth = args[i + 1];
                System.out.println("[usaged] teMonth: " + teMonth);
                i = i + 2;
                continue;
            }

            if (args[i].equals("-teday")) {
                teDay = args[i + 1];
                System.out.println("[usaged] teDay: " + teDay);
                i = i + 2;
                continue;
            }
            ...
            if (args[i].equals("-month")) {
                theMonth = args[i + 1];
                System.out.println("[usaged] theMonth: " + theMonth);
                i = i + 2;
                continue;
            }
            if (args[i].equals("-day")) {
                theDay = args[i + 1];
                System.out.println("[usaged] theDay: " + theDay);
                i = i + 2;
                continue;
            }
            ...
            System.out.println ("Unknown argument: " + args[i] +  "ignored.");
            i++;
        } // end of while
...
//      System.exit(0);
    ...
    @Override
    public void run() {
        System.out.println("[usageReportD] *** top of run ***");
        ...
        if (theYear == null && theMonth == null && theDay == null) {
            ...
            startMonth = Integer.valueOf("3");
            mystartDay = Integer.valueOf("1");
        ...
        try {
            System.out.println("[usageReportD] *** top of try ***");
            Logger LOG = Logger.getLogger("org.hibernate");
            LOG.setLevel(Level.ERROR);

            dataSource = new BatchDataSource(orionPath, schemaPath);
            app.connect();
            System.out.println("[usageReportD] after app.connect");
            ...
            if (toList == null) {
            } // property not set, use system.out for output
            ...
            System.out.println("[usageReportD] site_title: " + site_title);

            SimpleDateFormat sdf = new SimpleDateFormat("MMM dd, yyyy");
            System.out.println("The year: " + theYear);
            int theyear = Integer.valueOf(theYear);
            startDate = new GregorianCalendar(Integer.valueOf(tsYear), Integer.valueOf(tsMonth) - 1, Integer.valueOf(tsDay), 0, 0, 0);
            ...
            endDate = new GregorianCalendar(Integer.valueOf(teYear), Integer.valueOf(teMonth) - 1, Integer.valueOf(teDay), 0, 0, 0);
            Date edate = endDate.getTime();

            theEndDate = sdf.format(edate);
            ...
            if (theYear == null) {
                ...
                subject = "GNomEx Usage Report for " + todaysDate + " through " + theEndDate;
            }
...
//    		  MailUtil.validateAndSendEmail(helper);
    ...
    private StringBuffer buildUserUsageTable(String todaysDate) {
        ...
        userUsageTable.append("<tr><td class='tableHeadTop'  colspan='6'><span class='fontClassLgeBold'>" + "GNomEx Usage Report for " + todaysDate + " through " + theEndDate + "</span></td></tr>");
    ...
    private StringBuffer buildNewAETable(String todaysDate) {
        ...
        newAETable.append("<tr><td class='tableHeadTop' colspan='3' ><span class='fontClassLgeBold'>" + "New Experiments/Analyses for " + todaysDate + " through " + theEndDate + "</span></td></tr>");
...
//          "Select count(*) from VisitLog where id"
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
            startDate = new GregorianCalendar(Integer.valueOf(tsYear), Integer.valueOf(tsMonth) - 1, Integer.valueOf(tsDay), 0, 0, 0);
            Date date = startDate.getTime();
            String theStart = sdf.format(date);

            endDate = new GregorianCalendar(Integer.valueOf(teYear), Integer.valueOf(teMonth) - 1, Integer.valueOf(teDay), 0, 0, 0);
            Date edate = endDate.getTime();
            String theEnd = sdf.format(edate);

            System.out.println("theStart: " + theStart);
            System.out.println("theEnd: " + theEnd);

            String userWeeklyTotalVisits = getInternalCounts(sess,"select count(*) from VisitLog where visitDateTime >= '" + theStart + "' and visitDateTime <= '" + theEnd + "' and idAppUser != -999999");
            String userWeeklyTotalGuestDownloads = getInternalCounts(sess,"select count(*) from TransferLog where startDateTime >= '" + theStart + "' and endDateTime <= '" + theEnd + "' and idAppUser != -999999");
            String userWeeklyTotalGuests = getInternalCounts(sess,"select count(distinct idAppUser) from TransferLog where startDateTime >= '" + theStart + "' and endDateTime <= '" + theEnd + "' and idAppUser != 999999");
            ...
            guestUsageTable.append("<td class='tableHeadTop' colspan='3'><span class='fontClassLgeBold'>Internal Visits and Downloads for " + todaysDate + " through " + theEndDate + "</span></td>");
            guestUsageTable.append("</tr>");
            ...
            guestUsageTable.append("<tr>");
            guestUsageTable.append("<td class='tableHeadBottom' colspan='1'><span class='fontClassBold'>" + "Visits by Users" + "</span></td>");
            guestUsageTable.append("<td class='tableHeadBottom' colspan='1'><span class='fontClassBold'>" + "Files Downloaded by Users" + "</span></td>");
            guestUsageTable.append("<td class='tableHeadBottom' colspan='1'><span class='fontClassBold'>" + "Users Who Downloaded" + "</span></td>");
            guestUsageTable.append("</tr>");

            // Internal Summary - Data
            guestUsageTable.append("<tr>");
            guestUsageTable.append("<td  align='right' colspan='1'><span class='fontClass'>" + userWeeklyTotalVisits + "</span></td>");
            guestUsageTable.append("<td  align='right' colspan='1'><span class='fontClass'>" + userWeeklyTotalGuestDownloads + "</span></td>");
            guestUsageTable.append("<td  align='right' colspan='1'><span class='fontClass'>" + userWeeklyTotalGuests + "</span></td>");
            guestUsageTable.append("</tr><br><br><br>");


            // Guest Usage Table
            guestUsageTable.append("<table class='table700'  cellpadding='5' cellspacing='0' border='1' bgcolor='#F5FAFE'>");
            guestUsageTable.append("<tr>");
            guestUsageTable.append("<td class='tableHeadTop' colspan='3'><span class='fontClassLgeBold'>External Visits and Downloads for " + todaysDate + " through " + theEndDate + "</span></td>");
            guestUsageTable.append("</tr>");
    ...
    public String getInternalCounts(Session sess, String theQuery) {
        System.out.println("[getInternalCounts] theQuery: " + theQuery);
        ...
        try {
            ...
            List results = sess.createQuery(theQuery).list();
            if (results.size() > 0) {
                theResult = "" + (Integer) results.get(0);
            }

            System.out.println("[getInternalCounts]  theResult: " + theResult);
            if (myConn != null) {
                try {
                    myConn.close();
                    myConn = null;
            ...
            System.out.println ("In getInternalCounts ex: " + ex.toString());
            ex.printStackTrace();
            return null;
        } finally {
            if (myConn != null) {
...
//        System.out.println ("J");
    ...
    private void getGuestUsageDetail() {
        ...
        System.out.println("[usageReportd] after guest query, rows: " + rows.size());
        for (Iterator i = rows.iterator(); i.hasNext(); ) {
            Object[] row = (Object[]) i.next();
            Integer idLab = (Integer) row[0];
            LabStats ls = labInfo.get(idLab);
