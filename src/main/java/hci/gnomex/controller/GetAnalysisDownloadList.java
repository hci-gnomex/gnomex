...
public Command execute() throws RollBackCommandException {
	...
	try {
		...
		if (idAnalysis != null && idAnalysis.intValue() == 0) {
			...
			StringBuffer buf = new StringBuffer("SELECT a from Analysis as a where a.number = '"
					+ analysisNumber.toUpperCase() + "'");
			...
			for (Iterator i = analysisNumbers.iterator(); i.hasNext();) {
				...
				for (Iterator i1 = directoryKeys.iterator(); i1.hasNext();) {
					...
					if (includeUploadStagingDir.equals("Y")) {
						...
						addExpandedFileNodes(autoCreate, baseDir, aNode, aNode, analysisNumber, key, dh,
						...
						addExpandedFileNodes(autoCreate, baseDir, aNode, dummyNode, analysisNumber, key, dh,
								knownAnalysisFileMap, fileMap, dataTrackMap, sess);
					}
					...
					for (Iterator i2 = theFiles.iterator(); i2.hasNext();) {
						...
						fdNode.setAttribute(
								"key",
								directoryName != null && directoryName.length() > 0 ? a.getKey(directoryName) : a
										.getKey());
...
public static void addExpandedFileNodes(boolean autocreate, String baseDir, Element analysisNode,
		Element analysisDownloadNode, String analysisNumber, String key, DictionaryHelper dh, Map knownAnalysisFileMap,
	...
	for (Iterator i1 = directoryKeys.iterator(); i1.hasNext();) {
		...
		if (theFiles != null && theFiles.size() > 0) {
			for (Iterator i2 = theFiles.iterator(); i2.hasNext();) {
				...
				String viewType = Constants.DOWNLOAD_ANALYSIS_SINGLE_FILE_SERVLET + "?idAnalysis="
