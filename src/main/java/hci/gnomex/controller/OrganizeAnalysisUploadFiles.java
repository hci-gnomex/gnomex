...
public class OrganizeAnalysisUploadFiles extends GNomExCommand implements Serializable {
    ...
    public void loadCommand(HttpServletRequest request, HttpSession session) {
        ...
        if (request.getParameter("idAnalysis") != null && !request.getParameter("idAnalysis").equals("")) {
            ...
            System.out.println ("[OAUF] idAnalysis: " + idAnalysis);
        } else {
            this.addInvalidField("idAnalysis", "idAnalysis is required");
            System.out.println ("[OAUF] ERROR --> NO IDANALYSIS <--");
        }
        ...
        if (request.getParameter("filesXMLString") != null && !request.getParameter("filesXMLString").equals("")) {
            ...
            String filesXMLString1 = filesXMLString;
            if (filesXMLString.length() > 10000)
            {
                filesXMLString1 = filesXMLString1.substring(0,10000) + "\n";
            }
//            System.out.println ("[OAUF] filesXMLString: \n" + filesXMLString1 + "\n");
...
/*
                            if (parser.getFileIdMap().containsKey(fileName)) {
                                ...
                                if (idFileString != null) {
                                    ...
                                    af.setQualifiedFilePath(qualifiedFilePath);
                                    System.out.println("[OAUF] l 286 idFileString: " + idFileString + " fileName: " + fileName);
                                    sess.save(af);
                                }
                            }
*/
                            ...
                            // NOTE NOTE this code will never by executed!
...
/*
                                // why??? the file was transfered.  Just because it's deleted now that doesn't mean we should
                                // remove the information the TransferLog  11/29/2018 tim
...
*/
                                ...
                                System.out.println("[OAUF] l 417");
                                sess.flush();
                                System.out.println("[OAUF] after sess.flush l 423");
                            }
                        }
                    }
...
/*
                    // clean up ghost files         < --- I don't think we should do this!   11/29/2018 tim --- >
                    ...
                    for (Iterator i = ghostFiles.iterator(); i.hasNext(); ) {
                        ...
                        System.out.println("[OAUF] remove ghost file: L451: " + filePath);
                        analysis.getFiles().remove(af);
                    }
*/
                    System.out.println("[OAUF] l 455 *** at the end ***");
                    sess.flush();
