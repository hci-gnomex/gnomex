	...
	try {
		...
		if (body != null && body.length() > 0) {
			...
			for (Iterator i = appUsers.iterator(); i.hasNext();) {
				...
				MailUtilHelper helper = new MailUtilHelper(emailRecipients, fromAddress, subject, body.toString(),
						null, format.equalsIgnoreCase("HTML") ? true : false, dh, serverName);
		...
		throw new ServletException(
				"Unable to send broadcast email due to a server error.  Please contact GNomEx support.");
	} finally {
		try {
