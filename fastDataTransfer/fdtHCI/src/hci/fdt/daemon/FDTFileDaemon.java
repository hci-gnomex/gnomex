...
public class FDTFileDaemon {
  ...
  private static final String              UPLOAD_STAGING_DIR  = "upload_staging";
  ...
  private static boolean moveFiles(String sourceDir, String targetDir, boolean isTopLevel) {
    boolean isFinished = false;
    ...
    for( int x = 0; x < fileList.length; x++) {
      ...
      if (f.isDirectory()) {
        ...
        if (!new File(targetFileName).exists()) {
          ...
          if (!success) {
            System.out.println("Error: unable to create target directory: " + targetFileName);
            continue;      
          }
        }
        */ 
