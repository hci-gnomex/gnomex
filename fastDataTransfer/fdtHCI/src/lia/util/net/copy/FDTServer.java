/*
 * $Id: FDTServer.java,v 1.1 2012-10-29 22:29:38 HCI\rcundick Exp $
 */
...
public class FDTServer extends AbstractFDTCloseable {
				    ...
				    case 1: {
	                    ...
	                    while (clientIDBuff.hasRemaining()) {
	                        ...
	                        if (clientIDBuff.hasRemaining()) {
	                            // THIS CANNOT?!? happen in blocking mode ( JVM should throw exception if this happens ... but )
	                            if (config.isBlocking()) {
	...
	public FDTServer() throws Exception {
		...
		executor = Utils.getStandardExecService("[ Acceptable ServersThreadPool ] ", 5, 10, new ArrayBlockingQueue<Runnable>(65500), Thread.NORM_PRIORITY - 2);
		port = config.getPort();
		init();
	...
	public static final boolean filterSourceAddress(java.net.Socket socket) {
		/**
		 * check if remote client is allowed (based on optional filter specified in command line) if the address does not match, reject this IP (i.e DO NOT accept the session-tag
		 * from this client)
		 */
