package views.util
{
	...
	public class AdvancedDataGridGroupedRowColorsIdRequestAndMultiplexGroup extends AdvancedDataGrid
	{
		...
		override protected function drawRowBackground(s:Sprite, rowIndex:int, 
			...
			if (dataProvider != null && dataProvider.length > 0) {	// do we have a dataProvider or are we initializing an empty grid?			
				...
				if( dataIndex < dataProvider.length ) { 
					...
					if(item.hasOwnProperty("@idRequestAndMultiplexGroup") && item.@idRequestAndMultiplexGroup != ''){
						tempIdentifier = item.@idRequestAndMultiplexGroup;
