/*
 * $Id: ProcReader.java,v 1.1 2012-10-29 22:30:05 HCI\rcundick Exp $
 */
		...
		try {
			...
			while (en.hasMoreElements()) {
				...
				if (!ni.isLoopback()) {
					...
					} catch (SocketException se) { } // nothing, just move on here
				}
	...
	public HashMap<String, Double> getMEM() throws Exception {
		...
		while (line != null) {
			if (line.startsWith("MemTotal:")) {
				...
				StringTokenizer ast = new StringTokenizer(line);
				try { sMemTotal = ast.nextToken(); } catch (Exception e) { sMemTotal = null; } 
				double d = 0.0;
				try {
			...
			if (line.startsWith("MemFree:")) {
				...
				StringTokenizer ast = new StringTokenizer(line);
				try { sMemFree = ast.nextToken(); } catch (Exception e) { sMemFree = null; }
				double d = 0.0;
				try {
	...
	public HashMap<String, Double> getDISK() throws Exception {
		...
		if (output != null && !output.equals("")) {
			...
			if (line != null) {
				...
				while (true) {
					try { str = st.nextToken(" \t\n"); } catch (Exception e) { str = null; }
					if (str == null)
						break;
					final String device = str;
					devices.addLast(device);
					try { str = st.nextToken(" \t\n"); } catch (Exception e) { str = null; } // tps
					tps = addWithOverflowCheck(str, tps);
					try { str = st.nextToken(" \t\n"); } catch (Exception e) { str = null; } // skip KB read / sec
					try { str = st.nextToken(" \t\n"); } catch (Exception e) { str = null; } // skip KB write / sec
					try { str = st.nextToken(" \t\n"); } catch (Exception e) { str = null; } // KB read
					...
					// sec
					try { str = st.nextToken(" \t\n"); } catch (Exception e) { str = null; } // skip KB write /
		...
		if (output != null && output != "") {
			...
			while (true) {
				...
				if (line == null)
					break;
				try { line = st.nextToken(" \t\n"); } catch (Exception e) { line = null; } // size
				double d = 0.0;
				try {
				...
				if (d < 0.0)
					break;
				size += d;
				try { line = st.nextToken(" \t\n"); } catch (Exception e) { line = null; } // used
				d = 0.0;
				try {
					d = Double.parseDouble(line);
				} catch (Exception e) {
				...
				if (d < 0.0)
					break;
				used += d;
				try { line = st.nextToken(" \t\n"); } catch (Exception e) { line = null; } // available
				d = 0.0;
				try {
					d = Double.parseDouble(line);
				} catch (Exception e) {
				...
				if (d < 0.0)
					break;
				available += d;
				try { line = st.nextToken(" \t\n"); } catch (Exception e) { line = null; } // usage
				try { line = line.substring(0,line.indexOf("%")); } catch (Exception e) { } 
				d = 0.0;
				try {
					d = Double.parseDouble(line);
				} catch (Exception e) {
...
//			diskUsage = (usage * 1.0 / nr); // usage (%)
	...
	public HashMap<String, Double> getLOAD() throws Exception {
		...
		try { line = st.nextToken(" \t\n"); } catch (Exception e) { line = null; } // load1
		if (line != null) {
			double d = 0.0;
			try {
				d = Double.parseDouble(line);
			...
			if (d >= 0.0)
				load1 = d;
			try { line = st.nextToken(" \t\n"); } catch (Exception e) { line = null; }// load5
			d = 0.0;
			try {
				d = Double.parseDouble(line);
			} catch (Exception e) {
			...
			if (d >= 0.0)
				load5 = d;
			try { line = st.nextToken(" \t\n");} catch (Exception e) { line = null; } // load15
			d = 0.0;
			try {
				d = Double.parseDouble(line);
			} catch (Exception e) {
		...
		if (netInterfaces == null) {
			while (true) {
				...
				if (line.startsWith("eth") || line.startsWith("lo")) {
					...
					String name = line;
					try { line = st.nextToken(" \t\n"); } catch (Exception e) { line = null; } // bytes received
					...
					try { line = st.nextToken(" \t\n"); } catch (Exception e) { line = null; } // bytes sent
