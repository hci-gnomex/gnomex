...
public class XorOperator extends AbstractAnnotationOperator implements Operator
{
    ...
    public SeqSymmetry operate(final BioSeq aseq, final List<SeqSymmetry> symList) {
        return getXor(aseq, AbstractAnnotationOperator.findChildSyms(symList.get(0)), AbstractAnnotationOperator.findChildSyms(symList.get(1)));
    }
    
    protected static SeqSymmetry getXor(final BioSeq seq, final List<SeqSymmetry> symsA, final List<SeqSymmetry> symsB) {
        MutableSeqSymmetry psym = new SimpleSymWithProps();
        final SeqSymmetry unionA = SeqSymSummarizer.getUnion(symsA, seq);
        final SeqSymmetry unionB = SeqSymSummarizer.getUnion(symsB, seq);
